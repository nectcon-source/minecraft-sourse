...
public class RealmsServer extends ValueObject {
      ...
      for(String var5 : var1.players) {
         if (!var5.equals(Minecraft.getInstance().getUser().getUuid())) {
            ...
            try {
               ...
               LOGGER.error("Could not get name for " + var5, var7_1);
               continue;
            }
   ...
   public static RealmsServer parse(JsonObject var0) {
      ...
      try {
         ...
         LOGGER.error("Could not parse McoServer: " + var2_1.getMessage());
      }
   ...
   private static List<PlayerInfo> parseInvited(JsonArray var0) {
      List<PlayerInfo> var1 = Lists.newArrayList();
   ...
   private static Map<Integer, RealmsWorldOptions> parseSlots(JsonArray var0) {
      Map<Integer, RealmsWorldOptions> var1 = Maps.newHashMap();
   ...
   public static RealmsServer parse(String var0) {
      try {
         ...
         LOGGER.error("Could not parse McoServer: " + var1_1.getMessage());
   ...
   public Map<Integer, RealmsWorldOptions> cloneSlots(Map<Integer, RealmsWorldOptions> var1) {
      Map<Integer, RealmsWorldOptions> var2 = Maps.newHashMap();

      for(Map.Entry<Integer, RealmsWorldOptions> var4 : var1.entrySet()) {
