...
public class RealmsBackupScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      (new Thread("Realms-fetch-backups") {
         public void run() {
            ...
            try {
               ...
               RealmsBackupScreen.LOGGER.error("Couldn't request backups", var2_1);
            }

         }
      }).start();
      this.downloadButton = (Button)this.addButton(new Button(this.width - 135, row(1), 120, 20, new TranslatableComponent("mco.backup.button.download"), (var1) -> this.downloadClicked()));
      this.restoreButton = (Button)this.addButton(new Button(this.width - 135, row(3), 120, 20, new TranslatableComponent("mco.backup.button.restore"), (var1) -> this.restoreClicked(this.selectedBackup)));
      ...
      this.addButton(new Button(this.width - 100, this.height - 35, 85, 20, CommonComponents.GUI_BACK, (var1) -> this.minecraft.setScreen(this.lastScreen)));
   ...
   private void restoreClicked(int var1) {
      if (var1 >= 0 && var1 < this.backups.size() && !this.serverData.expired) {
         ...
         Component var5 = new TranslatableComponent("mco.configure.world.restore.question.line1", new Object[]{var3, var4});
         Component var6 = new TranslatableComponent("mco.configure.world.restore.question.line2");
         ...
         }, Type.Warning, var5, var6, true));
      }

   }

   private void downloadClicked() {
      Component var1 = new TranslatableComponent("mco.configure.world.restore.download.question.line1");
      Component var2 = new TranslatableComponent("mco.configure.world.restore.download.question.line2");
      this.minecraft.setScreen(new RealmsLongConfirmationScreen((var1x) -> {
         if (var1x) {
   ...
   class BackupObjectSelectionList extends RealmsObjectSelectionList<Entry> {
      ...
      public void selectItem(int var1) {
         ...
         if (var1 != -1) {
            NarrationHelper.now(I18n.get("narrator.select", new Object[]{((Backup)RealmsBackupScreen.this.backups.get(var1)).lastModifiedDate.toString()}));
         }
