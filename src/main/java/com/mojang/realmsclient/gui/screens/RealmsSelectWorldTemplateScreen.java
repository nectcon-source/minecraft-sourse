...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.trailerButton = (Button)this.addButton(new Button(this.width / 2 - 206, this.height - 32, 100, 20, new TranslatableComponent("mco.template.button.trailer"), (var1x) -> this.onTrailer()));
      this.selectButton = (Button)this.addButton(new Button(this.width / 2 - 100, this.height - 32, 100, 20, new TranslatableComponent("mco.template.button.select"), (var1x) -> this.selectTemplate()));
      Component var1 = this.worldType == RealmsServer.WorldType.MINIGAME ? CommonComponents.GUI_CANCEL : CommonComponents.GUI_BACK;
      Button var2 = new Button(this.width / 2 + 6, this.height - 32, 100, 20, var1, (var1x) -> this.backButtonClicked());
      this.addButton(var2);
      this.publisherButton = (Button)this.addButton(new Button(this.width / 2 + 112, this.height - 32, 100, 20, new TranslatableComponent("mco.template.button.publisher"), (var1x) -> this.onPublish()));
      ...
      Stream<Component> var3 = Stream.of(this.title);
      if (this.warning != null) {
         var3 = Stream.concat(Stream.of(this.warning), var3);
      }

      NarrationHelper.now(var3.filter(Objects::nonNull).map(Component::getString).collect(Collectors.toList()));
   }
   ...
   private void fetchTemplatesAsync(final WorldTemplatePaginatedList initialList) {
      new Thread("realms-template-fetcher") {
         @Override
         public void run() {
            WorldTemplatePaginatedList currentList = initialList;
            RealmsClient client = RealmsClient.create();

            while (currentList != null) {
               Either<WorldTemplatePaginatedList, String> fetchResult =
                       RealmsSelectWorldTemplateScreen.this.fetchTemplates(currentList, client);

               currentList = RealmsSelectWorldTemplateScreen.this.minecraft.submit(() -> {
                  if (fetchResult.right().isPresent()) {
                     // Обработка ошибки
                     RealmsSelectWorldTemplateScreen.LOGGER.error(
                             "Couldn't fetch templates: {}",
                             fetchResult.right().get());
                     ...
                     if (RealmsSelectWorldTemplateScreen.this.worldTemplateObjectSelectionList.isEmpty()) {
                        RealmsSelectWorldTemplateScreen.this.noTemplatesMessage =
                                TextRenderingUtils.decompose(
                                        ...
                                        new TextRenderingUtils.LineSegment[0]);
                     }
                     return null;
                  } else {
                     // Обработка успешного результата
                     WorldTemplatePaginatedList resultList = fetchResult.left().get();

                     for (WorldTemplate template : resultList.templates) {
                        RealmsSelectWorldTemplateScreen.this.worldTemplateObjectSelectionList.addEntry(template);
                     }

                     if (resultList.templates.isEmpty()) {
                        if (RealmsSelectWorldTemplateScreen.this.worldTemplateObjectSelectionList.isEmpty()) {
                           String message = I18n.get("mco.template.select.none", "%link");
                           TextRenderingUtils.LineSegment link = TextRenderingUtils.LineSegment.link(
                                   I18n.get("mco.template.select.none.linkTitle"),
                                   "https://aka.ms/MinecraftRealmsContentCreator");
                           RealmsSelectWorldTemplateScreen.this.noTemplatesMessage =
                                   TextRenderingUtils.decompose(message, link);
                        }
                        return null;
                     } else {
                        return resultList;
   ...
   private Either<WorldTemplatePaginatedList, String> fetchTemplates(WorldTemplatePaginatedList var1, RealmsClient var2) {
      ...
      } catch (RealmsServiceException var3_1) {
         return Either.right(var3_1.getMessage());
      }
   }
   ...
   public void render(PoseStack var1, int var2, int var3, float var4) {
      ...
      if (this.displayWarning) {
         ...
         for(int var10 = 0; var10 < var5.length; ++var10) {
            Component var11 = var5[var10];
            int var12 = 10526880;
            if (this.warningURL != null) {
               if (this.hoverWarning) {
                  var12 = 7107012;
                  var11 = var11.copy().withStyle(ChatFormatting.STRIKETHROUGH);
               } else {
                  var12 = 3368635;
               }
            }

            drawCenteredString(var1, this.font, var11, this.width / 2, row(-1 + var10), var12);
         }
      }
   ...
   class WorldTemplateObjectSelectionList extends RealmsObjectSelectionList<RealmsSelectWorldTemplateScreen.Entry> {
      ...
      public void selectItem(int var1) {
         ...
         if (var1 != -1) {
            ...
            String var3 = I18n.get("narrator.select.list.position", new Object[]{var1 + 1, RealmsSelectWorldTemplateScreen.this.worldTemplateObjectSelectionList.getItemCount()});
            String var4 = I18n.get("mco.template.select.narrate.version", new Object[]{var2.version});
            String var5 = I18n.get("mco.template.select.narrate.authors", new Object[]{var2.author});
            String var6 = NarrationHelper.join(Arrays.asList(var2.name, var5, var2.recommendedPlayers, var4, var3));
            NarrationHelper.now(I18n.get("narrator.select", new Object[]{var6}));
         }
      }
