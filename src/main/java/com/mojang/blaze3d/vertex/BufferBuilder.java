...
public class BufferBuilder extends DefaultedVertexConsumer implements BufferVertexConsumer {
   ...
   private void ensureCapacity(int var1) {
      if (this.nextElementByte + var1 > this.buffer.capacity()) {
         ...
         int var3x = var2 + roundUp(var1);
         LOGGER.debug("Needed to grow BufferBuilder buffer: Old size {} bytes, new size {} bytes.", var2, var3x);
         ByteBuffer var4xx = MemoryTracker.createByteBuffer(var3x);
         ((Buffer)this.buffer).position(0);
         var4xx.put(this.buffer);
         ((Buffer)var4xx).rewind();
         this.buffer = var4xx;
      }
   }
   ...
   public void sortQuads(float var1, float var2, float var3) {
      ...
      IntArrays.mergeSort(var15, (var1x, var2x) -> Floats.compare(var6[var2x], var6[var1x]));
   ...
   public State getState() {
      ...
      this.buffer.clear();
      return new State(var1, this.format);
   }
   ...
   public void nextElement() {
      ImmutableList<VertexFormatElement> var1 = this.format.getElements();
      this.elementIndex = (this.elementIndex + 1) % var1.size();
      this.nextElementByte += this.currentElement.getByteSize();
      VertexFormatElement var2x = (VertexFormatElement)var1.get(this.elementIndex);
      this.currentElement = var2x;
      if (var2x.getUsage() == VertexFormatElement.Usage.PADDING) {
         this.nextElement();
      }
   ...
   public Pair<DrawState, ByteBuffer> popNextBuffer() {
      DrawState var1 = (DrawState)this.vertexCounts.get(this.lastRenderedCountIndex++);
