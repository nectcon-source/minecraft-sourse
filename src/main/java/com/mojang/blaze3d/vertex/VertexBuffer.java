...
public class VertexBuffer implements AutoCloseable {
   ...
   public VertexBuffer(VertexFormat var1) {
      this.format = var1;
      RenderSystem.glGenBuffers(var1x -> this.id = var1x);
   }
   ...
   public void upload(BufferBuilder var1) {
      if (!RenderSystem.isOnRenderThread()) {
         RenderSystem.recordRenderCall(() -> this.upload_(var1));
      } else {
         this.upload_(var1);
      }
   }
   ...
   public CompletableFuture<Void> uploadLater(BufferBuilder var1) {
      if (!RenderSystem.isOnRenderThread()) {
         return CompletableFuture.runAsync(() -> this.upload_(var1), var0 -> RenderSystem.recordRenderCall(var0::run));
      } else {
         this.upload_(var1);
   ...
   private void upload_(BufferBuilder var1) {
      Pair<BufferBuilder.DrawState, ByteBuffer> var2 = var1.popNextBuffer();
      if (this.id != -1) {
         ByteBuffer var3x = (ByteBuffer)var2.getSecond();
         this.vertexCount = var3x.remaining() / this.format.getVertexSize();
         this.bind();
         RenderSystem.glBufferData(34962, var3x, 35044);
         unbind();
      }
   }
