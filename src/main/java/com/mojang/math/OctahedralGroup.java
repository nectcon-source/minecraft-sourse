...
public enum OctahedralGroup implements StringRepresentable {
   ...
   private static final OctahedralGroup[][] cayleyTable = (OctahedralGroup[][])Util.make(new OctahedralGroup[values().length][values().length], (var0) -> {
      Map<Pair<SymmetricGroup3, BooleanList>, OctahedralGroup> var1 = (Map)Arrays.stream(values()).collect(Collectors.toMap((var0x) -> Pair.of(var0x.permutation, var0x.packInversions()), (var0x) -> var0x));
   ...
   public Direction rotate(Direction var1) {
      if (this.rotatedDirections == null) {
         ...
         for(Direction var5 : Direction.values()) {
            ...
            Direction.Axis var8 = Axis.values()[this.permutation.permutation(var6.ordinal())];
