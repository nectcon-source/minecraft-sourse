...
//      List<T> var3 = Lists.newArrayList();
//
//      while(this.reader.peek() != ']') {
//         int var4x = this.reader.getCursor();
//         Tag var5xx = this.readValue();
//         TagType<?> var6xxx = var5xx.getType();
//         if (var6xxx != var2) {
//            this.reader.setCursor(var4x);
//            throw ERROR_INSERT_MIXED_ARRAY.createWithContext(this.reader,var6xxx.getPrettyName(), var1.getPrettyName());
//         }
//
//         if (var2 == ByteTag.TYPE) {
//            var3.add((T)((NumericTag)var5xx).getAsByte());
//         } else if (var2 == LongTag.TYPE) {
//            var3.add((T)((NumericTag)var5xx).getAsLong());
//         } else {
//            var3.add((T)((NumericTag)var5xx).getAsInt());
//         }
//
//         if (!this.hasElementSeparator()) {
//            break;
...
//      return var3;
//   }
private <T extends Number> List<T> readArray(TagType<?> expectedType, TagType<?> elementType)
   ...
   while (this.reader.peek() != ']') {
      ...
      if (actualType != elementType) {
         ...
         throw ERROR_INSERT_MIXED_ARRAY.createWithContext(
                 this.reader,
                 actualType.getPrettyName(),
                 expectedType.getPrettyName()
         );
      }
      ...
      // Безопасное приведение через Class.cast()
