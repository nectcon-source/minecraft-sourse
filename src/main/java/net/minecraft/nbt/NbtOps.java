...
public class NbtOps implements DynamicOps<Tag> {
   ...
   private static <T extends Tag> void fillOne(CollectionTag<T> var0, Tag var1, Tag var2) {
      if (var1 instanceof CollectionTag) {
         ...
         var3.forEach((var1x) -> var0.add((T) var1x));
      }
   ...
   public DataResult<Tag> mergeToMap(Tag var1, MapLike<Tag> var2) {
      if (!(var1 instanceof CompoundTag) && !(var1 instanceof EndTag)) {
         ...
         List<Tag> var5 = Lists.newArrayList();
   ...
   public DataResult<Consumer<BiConsumer<Tag, Tag>>> getMapEntries(Tag var1) {
      if (!(var1 instanceof CompoundTag)) {
         ...
         CompoundTag var2 = (CompoundTag)var1;
         return DataResult.success((var2x) -> var2.getAllKeys().forEach((var3) -> var2x.accept(this.createString(var3), var2.get(var3))));
      }
   }
      ...
      if (!(var1 instanceof CompoundTag)) {
         ...
         return DataResult.success(new MapLike<Tag>() {
            ...
            public Stream<Pair<Tag, Tag>> entries() {
               return var2.getAllKeys().stream().map((var2x) -> Pair.of(NbtOps.this.createString(var2x), var2.get(var2x)));
            }
   ...
   public Tag createList(Stream<Tag> var1) {
      PeekingIterator<Tag> var2 = Iterators.peekingIterator(var1.iterator());
      if (!var2.hasNext()) {
         return new ListTag();
      } else {
         Tag var3 = (Tag)var2.peek();
         if (var3 instanceof ByteTag) {
            List<Byte> var8 = Lists.newArrayList(Iterators.transform(var2, (var0) -> ((ByteTag)var0).getAsByte()));
            return new ByteArrayTag(var8);
         } else if (var3 instanceof IntTag) {
            List<Integer> var7 = Lists.newArrayList(Iterators.transform(var2, (var0) -> ((IntTag)var0).getAsInt()));
            return new IntArrayTag(var7);
         } else if (var3 instanceof LongTag) {
            List<Long> var6 = Lists.newArrayList(Iterators.transform(var2, (var0) -> ((LongTag)var0).getAsLong()));
      ...
      protected DataResult<Tag> build(CompoundTag var1, Tag var2) {
         if (var2 != null && var2 != EndTag.INSTANCE) {
            if (!(var2 instanceof CompoundTag)) {
               ...
               for(Map.Entry<String, Tag> var5 : var1.entries().entrySet()) {
