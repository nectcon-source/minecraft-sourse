...
public class MobEffectsPredicate {
   ...
   public boolean matches(Map<MobEffect, MobEffectInstance> var1) {
      if (this == ANY) {
         ...
         for(Map.Entry<MobEffect, MobEffectInstancePredicate> var3 : this.effects.entrySet()) {
   ...
   public static MobEffectsPredicate fromJson(@Nullable JsonElement var0) {
      if (var0 != null && !var0.isJsonNull()) {
         ...
         Map<MobEffect, MobEffectInstancePredicate> var2 = Maps.newLinkedHashMap();

         for(Map.Entry<String, JsonElement> var4 : var1.entrySet()) {
            ResourceLocation var5 = new ResourceLocation((String)var4.getKey());
            MobEffect var6 = (MobEffect)Registry.MOB_EFFECT.getOptional(var5).orElseThrow(() -> new JsonSyntaxException("Unknown effect '" + var5 + "'"));
   ...
   public JsonElement serializeToJson() {
      if (this == ANY) {
         ...
         for(Map.Entry<MobEffect, MobEffectInstancePredicate> var3 : this.effects.entrySet()) {
            var1.add(Registry.MOB_EFFECT.getKey(var3.getKey()).toString(), ((MobEffectInstancePredicate)var3.getValue()).serializeToJson());
         }
