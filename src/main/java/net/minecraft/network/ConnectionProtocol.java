...
public enum ConnectionProtocol {
   ...
   public Packet<?> createPacket(PacketFlow var1, int var2) {
      return this.flows.get(var1).createPacket(var2);
   }
   ...
   static class PacketSet<T extends PacketListener> {
      private final Object2IntMap<Class<? extends Packet<T>>> classToId = Util.make(new Object2IntOpenHashMap(), var0 -> var0.defaultReturnValue(-1));
      ...
      public <P extends Packet<T>> ConnectionProtocol.PacketSet<T> addPacket(Class<P> var1, Supplier<P> var2) {
         ...
         int var4x = this.classToId.put(var1, var3);
         if (var4x != -1) {
            String var5xx = "Packet " + var1 + " is already registered to ID " + var4x;
            LogManager.getLogger().fatal(var5xx);
            throw new IllegalArgumentException(var5xx);
         } else {
      ...
      public Packet<?> createPacket(int var1) {
         Supplier<? extends Packet<T>> var2 = this.idToConstructor.get(var1);
