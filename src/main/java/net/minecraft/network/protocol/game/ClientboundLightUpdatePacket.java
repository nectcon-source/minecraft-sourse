...
public class ClientboundLightUpdatePacket implements Packet<ClientGamePacketListener> {
   ...
   public ClientboundLightUpdatePacket(ChunkPos var1, LevelLightEngine var2, int var3, int var4, boolean var5) {
      ...
      for(int var6 = 0; var6 < 18; ++var6) {
         if ((this.skyYMask & 1 << var6) != 0) {
            DataLayer var7 = var2.getLayerListener(LightLayer.SKY).getDataLayerData(SectionPos.of(var1, -1 + var6));
         ...
         if ((this.blockYMask & 1 << var6) != 0) {
            DataLayer var8 = var2.getLayerListener(LightLayer.BLOCK).getDataLayerData(SectionPos.of(var1, -1 + var6));
            if (var8 != null && !var8.isEmpty()) {
               this.blockUpdates.add(var8.getData().clone());
            } else {
   ...
   public void write(FriendlyByteBuf var1) throws IOException {
      ...
      for(byte[] var3 : this.skyUpdates) {
         var1.writeByteArray(var3);
      }

      for(byte[] var5 : this.blockUpdates) {
