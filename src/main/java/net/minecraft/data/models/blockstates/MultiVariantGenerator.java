...
public class MultiVariantGenerator implements BlockStateGenerator {
   ...
   public JsonElement get() {
      Stream<Pair<Selector, List<Variant>>> var1 = Stream.of(Pair.of(Selector.empty(), this.baseVariants));

      for(PropertyDispatch var3 : this.declaredPropertySets) {
         Map<Selector, List<Variant>> var4x = var3.getEntries();
         var1 = var1.flatMap(var1x -> var4x.entrySet().stream().map(var1xx -> {
               Selector var2 = ((Selector)var1x.getFirst()).extend((Selector)var1xx.getKey());
               List<Variant> var3x = mergeVariants((List<Variant>)var1x.getSecond(), (List<Variant>)var1xx.getValue());
               return Pair.of(var2, var3x);
            }));
      }

      Map<String, JsonElement> var5x = new TreeMap<>();
      var1.forEach(var1x -> ((Selector)var1x.getFirst()).getKey());
      JsonObject var6xx = new JsonObject();
      var6xx.add("variants", Util.make(new JsonObject(), var1x -> var5x.forEach(var1x::add)));
      return var6xx;
   }

   private static List<Variant> mergeVariants(List<Variant> var0, List<Variant> var1) {
      ImmutableList.Builder<Variant> var2 = ImmutableList.builder();
