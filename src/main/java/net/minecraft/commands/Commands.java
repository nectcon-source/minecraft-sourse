...
public class Commands {
   ...
   public Commands(Commands.CommandSelection var1) {
      ...
      this.dispatcher.setConsumer((var0, var1x, var2) -> ((CommandSourceStack)var0.getSource()).onCommandComplete(var0, var1x, var2));
   }
   ...
   public int performCommand(CommandSourceStack var1, String var2) {
      ...
      try {
         try {
            ...
            return var4;
         } catch (CommandRuntimeException var4_23) {
            var1.sendFailure(var4_23.getComponent());
            byte var19 = 0;
            return var19;
         } catch (CommandSyntaxException var4_14) {
            var1.sendFailure(ComponentUtils.fromMessage(var4_14.getRawMessage()));
            if (var4_14.getInput() != null && var4_14.getCursor() >= 0) {
               int var17 = Math.min(var4_14.getInput().length(), var4_14.getCursor());
               MutableComponent var21 = (new TextComponent("")).withStyle(ChatFormatting.GRAY).withStyle((var1x) -> var1x.withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, var2)));
               if (var17 > 10) {
                  var21.append("...");
               }

               var21.append(var4_14.getInput().substring(Math.max(0, var17 - 10), var17));
               if (var17 < var4_14.getInput().length()) {
                  Component var22 = (new TextComponent(var4_14.getInput().substring(var17))).withStyle(new ChatFormatting[]{ChatFormatting.RED, ChatFormatting.UNDERLINE});
                  var21.append(var22);
               }

               var21.append((new TranslatableComponent("command.context.here")).withStyle(new ChatFormatting[]{ChatFormatting.RED, ChatFormatting.ITALIC}));
               var1.sendFailure(var21);
            }
         } catch (Exception var15) {
            MutableComponent var5 = new TextComponent(var15.getMessage() == null ? var15.getClass().getName() : var15.getMessage());
            ...
            var1.sendFailure((new TranslatableComponent("command.failed")).withStyle((var1x) -> var1x.withHoverEvent(new HoverEvent(net.minecraft.network.chat.HoverEvent.Action.SHOW_TEXT, var5))));
            if (SharedConstants.IS_RUNNING_IN_IDE) {
   ...
   private void fillUsableCommands(CommandNode<CommandSourceStack> var1, CommandNode<SharedSuggestionProvider> var2, CommandSourceStack var3, Map<CommandNode<CommandSourceStack>, CommandNode<SharedSuggestionProvider>> var4) {
      for(CommandNode<CommandSourceStack> var6 : var1.getChildren()) {
         if (var6.canUse(var3)) {
            ArgumentBuilder<SharedSuggestionProvider, ?> var7 = (ArgumentBuilder<SharedSuggestionProvider, ?>) (ArgumentBuilder<?, ?>)var6.createBuilder();
            ...
            if (var7 instanceof RequiredArgumentBuilder) {
               RequiredArgumentBuilder<SharedSuggestionProvider, ?> var8 = (RequiredArgumentBuilder<SharedSuggestionProvider, ?>) var7;
            ...
            CommandNode<SharedSuggestionProvider> var9 = var7.build();
            var4.put(var6, var9);
            var2.addChild(var9);
   ...
   public static void validate() {
      ...
      Set<ArgumentType<?>> var2 = var1.stream().filter((var0x) -> !ArgumentTypes.isTypeRegistered(var0x)).collect(Collectors.toSet());
      if (!var2.isEmpty()) {
         LOGGER.warn("Missing type registration for following arguments:\n {}", var2.stream().map((var0x) -> "\t" + var0x).collect(Collectors.joining(",\n")));
