   ...
   public Coordinates parse(StringReader var1) throws CommandSyntaxException {
      ...
      if (!var1.canRead()) {
         ...
         if (var1.canRead() && var1.peek() == ' ') {
            ...
            return new WorldCoordinates(var3, new WorldCoordinate(true, (double)0.0F), var4);
         } else {
            var1.setCursor(var2);
            throw ERROR_NOT_COMPLETE.createWithContext(var1);
         }
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> var1, SuggestionsBuilder var2) {
      if (!(var1.getSource() instanceof SharedSuggestionProvider)) {
         ...
         if (!var3.isEmpty() && var3.charAt(0) == '^') {
            var4 = Collections.singleton(TextCoordinates.DEFAULT_LOCAL);
         } else {
            var4 = ((SharedSuggestionProvider)var1.getSource()).getAbsoluteCoordinates();
         }

         return SharedSuggestionProvider.suggest2DCoordinates(var3, var4, var2, Commands.createValidator(this::parse));
      }
   }
