...
public class ItemPredicateArgument implements ArgumentType<ItemPredicateArgument.Result> {
   ...
   private static final DynamicCommandExceptionType ERROR_UNKNOWN_TAG = new DynamicCommandExceptionType((var0) -> new TranslatableComponent("arguments.item.tag.unknown", new Object[]{var0}));
   ...
   public Result parse(StringReader var1) throws CommandSyntaxException {
      ...
      if (var2.getItem() != null) {
         ItemPredicate var4 = new ItemPredicate(var2.getItem(), var2.getNbt());
         return (var1x) -> var4;
      } else {
         ResourceLocation var3 = var2.getTag();
         return (var2x) -> {
            Tag<Item> var3x = ((CommandSourceStack)var2x.getSource()).getServer().getTags().getItems().getTag(var3);
            if (var3x == null) {
               throw ERROR_UNKNOWN_TAG.create(var3.toString());
            } else {
               return new TagPredicate(var3x, var2.getNbt());
            }
         };
      }
   }
