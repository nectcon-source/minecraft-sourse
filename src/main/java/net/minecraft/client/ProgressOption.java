...
public class ProgressOption extends Option {
   ...
   public double toPct(double var1) {
      return Mth.clamp((this.clamp(var1) - this.minValue) / (this.maxValue - this.minValue), (double)0.0F, (double)1.0F);
   }

   public double toValue(double var1) {
      return this.clamp(Mth.lerp(Mth.clamp(var1, (double)0.0F, (double)1.0F), this.minValue, this.maxValue));
   }

   private double clamp(double var1) {
      if (this.steps > 0.0F) {
         var1 = (this.steps * (float)Math.round(var1 / (double)this.steps));
      }
