...
public class RemotePlayer extends AbstractClientPlayer {
   ...
   public boolean shouldRenderAtSqrDistance(double var1) {
      double var3 = this.getBoundingBox().getSize() * (double)10.0F;
      if (Double.isNaN(var3)) {
         var3 = (double)1.0F;
      }

      var3 *= (double)64.0F * getViewScale();
   ...
   public void aiStep() {
      ...
      if (!this.onGround && !this.isDeadOrDying()) {
         float var8 = (float)Math.atan(-this.getDeltaMovement().y * (double)0.2F) * 15.0F;
      } else {
         float var2 = 0.0F;
      }
