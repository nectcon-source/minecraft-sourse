...
public class GuardianModel extends ListModel<Guardian> {
   ...
   public void setupAnim(Guardian var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      this.eye.z = -8.25F;
      Entity var9 = Minecraft.getInstance().getCameraEntity();
      if (var1.hasActiveAttackTarget()) {
         var9 = var1.getActiveAttackTarget();
      }

      if (var9 != null) {
         Vec3 var10 = var9.getEyePosition(0.0F);
         ...
         var14 = new Vec3(var14.x, (double)0.0F, var14.z);
         Vec3 var15 = (new Vec3(var11.x - var10.x, (double)0.0F, var11.z - var10.z)).normalize().yRot(((float)Math.PI / 2F));
      ...
      this.tailParts[0].yRot = Mth.sin(var18) * (float)Math.PI * 0.05F;
      this.tailParts[1].yRot = Mth.sin(var18) * (float)Math.PI * 0.1F;
      this.tailParts[1].x = -1.5F;
      this.tailParts[1].y = 0.5F;
      this.tailParts[1].z = 14.0F;
      this.tailParts[2].yRot = Mth.sin(var18) * (float)Math.PI * 0.15F;
   ...
   private void setupSpikes(float var1, float var2) {
      for(int var3 = 0; var3 < 12; ++var3) {
         this.spikeParts[var3].xRot = (float)Math.PI * SPIKE_X_ROT[var3];
         this.spikeParts[var3].yRot = (float)Math.PI * SPIKE_Y_ROT[var3];
         this.spikeParts[var3].zRot = (float)Math.PI * SPIKE_Z_ROT[var3];
