...
public class WitherBossModel<T extends WitherBoss> extends ListModel<T> {
   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      this.upperBodyParts[1].xRot = (0.065F + 0.05F * var7) * (float)Math.PI;
      ...
      this.upperBodyParts[2].xRot = (0.265F + 0.1F * var7) * (float)Math.PI;
      this.heads[0].yRot = var5 * ((float)Math.PI / 180F);
      this.heads[0].xRot = var6 * ((float)Math.PI / 180F);
   }

   public void prepareMobModel(T var1, float var2, float var3, float var4) {
      for(int var5 = 1; var5 < 3; ++var5) {
         this.heads[var5].yRot = (var1.getHeadYRot(var5 - 1) - var1.yBodyRot) * ((float)Math.PI / 180F);
         this.heads[var5].xRot = var1.getHeadXRot(var5 - 1) * ((float)Math.PI / 180F);
      }
