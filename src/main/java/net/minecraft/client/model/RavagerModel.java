...
public class RavagerModel extends ListModel<Ravager> {
   ...
   public void setupAnim(Ravager var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      this.leg1.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI) * var7;
      this.leg2.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI) * var7;
   ...
   public void prepareMobModel(Ravager var1, float var2, float var3, float var4) {
      ...
      if (var8 > 0) {
         ...
         float var14 = Mth.sin(((float)var8 - var4) / 10.0F * (float)Math.PI * 0.25F);
         this.mouth.xRot = ((float)Math.PI / 2F) * var14;
         if (var8 > 5) {
            this.mouth.xRot = Mth.sin(((float)(-4 + var8) - var4) / 4.0F) * (float)Math.PI * 0.4F;
         } else {
            this.mouth.xRot = 0.15707964F * Mth.sin((float)Math.PI * ((float)var8 - var4) / 10.0F);
         }
      } else {
         float var15 = -1.0F;
         ...
         this.mouth.xRot = (float)Math.PI * (var17 ? 0.05F : 0.01F);
         if (var17) {
            double var18 = (double)var5 / (double)40.0F;
            this.neck.x = (float)Math.sin(var18 * (double)10.0F) * 3.0F;
         } else if (var6 > 0) {
            float var19 = Mth.sin(((float)(20 - var6) - var4) / 20.0F * (float)Math.PI * 0.25F);
            this.mouth.xRot = ((float)Math.PI / 2F) * var19;
         }
      }
