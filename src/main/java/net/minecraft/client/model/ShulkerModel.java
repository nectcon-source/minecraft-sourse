...
public class ShulkerModel<T extends Shulker> extends ListModel<T> {
   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      float var8 = (0.5F + var1.getClientPeekAmount(var7)) * (float)Math.PI;
      float var9 = -1.0F + Mth.sin(var8);
      float var10 = 0.0F;
      if (var8 > (float)Math.PI) {
         var10 = Mth.sin(var4 * 0.1F) * 0.7F;
      }

      this.lid.setPos(0.0F, 16.0F + Mth.sin(var8) * 8.0F + var10, 0.0F);
      if (var1.getClientPeekAmount(var7) > 0.3F) {
         this.lid.yRot = var9 * var9 * var9 * var9 * (float)Math.PI * 0.125F;
      } else {
         this.lid.yRot = 0.0F;
      }

      this.head.xRot = var6 * ((float)Math.PI / 180F);
      this.head.yRot = (var1.yHeadRot - 180.0F - var1.yBodyRot) * ((float)Math.PI / 180F);
   }
