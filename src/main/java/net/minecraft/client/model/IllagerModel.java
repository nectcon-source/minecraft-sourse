...
public class IllagerModel<T extends AbstractIllager> extends ListModel<T> implements ArmedModel, HeadedModel {
   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      if (this.riding) {
         ...
         this.rightArm.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI) * 2.0F * var3 * 0.5F;
         this.rightArm.yRot = 0.0F;
         this.rightArm.zRot = 0.0F;
         ...
         this.rightLeg.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI) * 1.4F * var3 * 0.5F;
         this.rightLeg.yRot = 0.0F;
         this.rightLeg.zRot = 0.0F;
      }

      AbstractIllager.IllagerArmPose var7 = var1.getArmPose();
      if (var7 == IllagerArmPose.ATTACKING) {
      ...
      } else if (var7 == IllagerArmPose.SPELLCASTING) {
         ...
         this.rightArm.yRot = 0.0F;
         this.leftArm.yRot = 0.0F;
      } else if (var7 == IllagerArmPose.BOW_AND_ARROW) {
         this.rightArm.yRot = -0.1F + this.head.yRot;
         this.rightArm.xRot = (-(float)Math.PI / 2F) + this.head.xRot;
         this.leftArm.xRot = -0.9424779F + this.head.xRot;
         this.leftArm.yRot = this.head.yRot - 0.4F;
         this.leftArm.zRot = ((float)Math.PI / 2F);
      } else if (var7 == IllagerArmPose.CROSSBOW_HOLD) {
         AnimationUtils.animateCrossbowHold(this.rightArm, this.leftArm, this.head, true);
      } else if (var7 == IllagerArmPose.CROSSBOW_CHARGE) {
         AnimationUtils.animateCrossbowCharge(this.rightArm, this.leftArm, var1, true);
      } else if (var7 == IllagerArmPose.CELEBRATING) {
         this.rightArm.z = 0.0F;
         this.rightArm.x = -5.0F;
      ...
      boolean var8 = var7 == IllagerArmPose.CROSSED;
