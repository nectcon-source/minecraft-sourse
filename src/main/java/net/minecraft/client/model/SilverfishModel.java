...
public class SilverfishModel<T extends Entity> extends ListModel<T> {
   ...
   public SilverfishModel() {
      ...
      ImmutableList.Builder<ModelPart> var3 = ImmutableList.builder();
   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      for(int var7 = 0; var7 < this.bodyParts.length; ++var7) {
         this.bodyParts[var7].yRot = Mth.cos(var4 * 0.9F + (float)var7 * 0.15F * (float)Math.PI) * (float)Math.PI * 0.05F * (float)(1 + Math.abs(var7 - 2));
         this.bodyParts[var7].x = Mth.sin(var4 * 0.9F + (float)var7 * 0.15F * (float)Math.PI) * (float)Math.PI * 0.2F * (float)Math.abs(var7 - 2);
      }
