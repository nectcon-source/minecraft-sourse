...
public class BatModel extends ListModel<Bat> {
   ...
   public void setupAnim(Bat var1, float var2, float var3, float var4, float var5, float var6) {
      if (var1.isResting()) {
         ...
         this.head.yRot = (float)Math.PI - var5 * ((float)Math.PI / 180F);
         ...
         this.body.xRot = ((float)Math.PI / 4F) + Mth.cos(var4 * 0.1F) * 0.15F;
         this.body.yRot = 0.0F;
         this.rightWing.yRot = Mth.cos(var4 * 1.3F) * (float)Math.PI * 0.25F;
         this.leftWing.yRot = -this.rightWing.yRot;
