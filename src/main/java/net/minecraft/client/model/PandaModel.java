...
public class PandaModel<T extends Panda> extends QuadrupedModel<T> {
   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      if (var8) {
         if (var9 < 15) {
            this.head.xRot = (-(float)Math.PI / 4F) * (float)var9 / 14.0F;
         } else if (var9 < 20) {
            float var12 = (float)((var9 - 15) / 5);
            this.head.xRot = (-(float)Math.PI / 4F) + ((float)Math.PI / 4F) * var12;
         }
      }
      ...
      if (this.sitAmount > 0.0F) {
         ...
         if (var10) {
            this.head.xRot = ((float)Math.PI / 2F) + 0.2F * Mth.sin(var4 * 0.6F);
