...
public class ParrotModel extends ListModel<Parrot> {
   ...
   public void renderOnShoulder(PoseStack var1, VertexConsumer var2, int var3, int var4, float var5, float var6, float var7, float var8, int var9) {
      ...
      this.parts().forEach((var4x) -> var4x.render(var1, var2, var3, var4));
   }

   private void setupAnim(State var1, int var2, float var3, float var4, float var5, float var6, float var7) {
      this.head.xRot = var7 * ((float)Math.PI / 180F);
      this.head.yRot = var6 * ((float)Math.PI / 180F);
      this.head.zRot = 0.0F;
      this.head.x = 0.0F;
      this.body.x = 0.0F;
      this.tail.x = 0.0F;
      ...
      switch (var1) {
         ...
         case STANDING:
            ...
            var10000.xRot += Mth.cos(var3 * 0.6662F + (float)Math.PI) * 1.4F * var4;
         case FLYING:
         case ON_SHOULDER:
         default:
