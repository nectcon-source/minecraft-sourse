   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      if (this.state != 3) {
         ...
         if (this.state == 2) {
            ...
            this.frontLegL.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI + 0.3F) * var3;
            this.frontLegR.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI) * var3;
            this.tail2.xRot = 1.7278761F + ((float)Math.PI / 10F) * Mth.cos(var2) * var3;
         } else {
            this.backLegL.xRot = Mth.cos(var2 * 0.6662F) * var3;
            this.backLegR.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI) * var3;
            this.frontLegL.xRot = Mth.cos(var2 * 0.6662F + (float)Math.PI) * var3;
            this.frontLegR.xRot = Mth.cos(var2 * 0.6662F) * var3;
            if (this.state == 1) {
               this.tail2.xRot = 1.7278761F + ((float)Math.PI / 4F) * Mth.cos(var2) * var3;
            } else {
