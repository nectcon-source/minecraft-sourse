...
public class BeeModel<T extends Bee> extends AgeableListModel<T> {
   ...
   public void setupAnim(T var1, float var2, float var3, float var4, float var5, float var6) {
      ...
      if (var7) {
         ...
         float var8 = var4 * 2.1F;
         this.rightWing.yRot = 0.0F;
         this.rightWing.zRot = Mth.cos(var8) * (float)Math.PI * 0.15F;
      ...
      if (!var1.isAngry()) {
         ...
         if (!var7) {
            ...
            this.bone.xRot = 0.1F + var9 * (float)Math.PI * 0.025F;
            this.leftAntenna.xRot = var9 * (float)Math.PI * 0.03F;
            this.rightAntenna.xRot = var9 * (float)Math.PI * 0.03F;
            this.frontLeg.xRot = -var9 * (float)Math.PI * 0.1F + ((float)Math.PI / 8F);
            this.backLeg.xRot = -var9 * (float)Math.PI * 0.05F + ((float)Math.PI / 4F);
