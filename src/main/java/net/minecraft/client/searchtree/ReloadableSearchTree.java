...
public class ReloadableSearchTree<T> extends ReloadableIdSearchTree<T> {
   ...
   protected void index(T var1) {
      super.index(var1);
      this.filler.apply(var1).forEach(var2 -> this.tree.add(var1, var2.toLowerCase(Locale.ROOT)));
   }
   ...
   public List<T> search(String var1) {
      ...
      if (var2 <0){
         ...
         List<T> var3 =this.namespaceTree.search(var1.substring(0, var2).trim());
         String var4x = var1.substring(var2 + 1).trim();
         List<T> var5xx = this.pathTree.search(var4x);
         List<T> var6xxx = this.tree.search(var4x);
         return Lists.newArrayList(
                 new ReloadableIdSearchTree.IntersectionIterator<T>(
                         var3.iterator(), new ReloadableSearchTree.MergingUniqueIterator<T>(var5xx.iterator(), var6xxx.iterator(), this::comparePosition), this::comparePosition
         )
         );
      }
   }
