...
public class ReloadableIdSearchTree<T> implements MutableSearchTree<T> {
   ...
   protected void index(T var1) {
      (this.idGetter.apply(var1)).forEach((var2) -> {
   ...
   public List<T> search(String var1) {
      ...
      if (var2 == -1) {
         ...
         return Lists.newArrayList(new ReloadableIdSearchTree.IntersectionIterator<T>(var3.iterator(), var5xx.iterator(), this::comparePosition));
      }
   }
