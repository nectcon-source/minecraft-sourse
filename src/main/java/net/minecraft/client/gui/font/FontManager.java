...
public class FontManager implements AutoCloseable {
   ...
   private final PreparableReloadListener reloadListener = new SimplePreparableReloadListener<Map<ResourceLocation, List<GlyphProvider>>>() {
      protected Map<ResourceLocation, List<GlyphProvider>> prepare(ResourceManager var1, ProfilerFiller var2) {
         ...
         Map<ResourceLocation, List<GlyphProvider>> var4 = Maps.newHashMap();

         for(ResourceLocation var6 : var1.listResources("font", (var0) -> var0.endsWith(".json"))) {
            ...
            List<GlyphProvider> var9 = (List)var4.computeIfAbsent(var8, (var0) -> Lists.newArrayList(new GlyphProvider[]{new AllMissingGlyphProvider()}));
            var2.push(var8::toString);

            try {
               for(Resource var11 : var1.getResources(var6)) {
                  var2.push(var11::getSourceName);

                  try (InputStream var12 = var11.getInputStream()) {
                     Reader var14 = new BufferedReader(new InputStreamReader(var12, StandardCharsets.UTF_8));
                     Throwable var15 = null;

                     try {
                        var2.push("reading");
                        JsonArray var16 = GsonHelper.getAsJsonArray((JsonObject)GsonHelper.fromJson(var3, var14, JsonObject.class), "providers");
                        ...
                        for(int var17 = var16.size() - 1; var17 >= 0; --var17) {
                           ...
                           try {
                              ...
                              var2.pop();
                           } catch (RuntimeException var19_1) {
                              FontManager.LOGGER.warn("Unable to read definition '{}' in fonts.json in resourcepack: '{}': {}", var8, var11.getSourceName(), var19_1.getMessage());
                           }
                        }
                  ...
                  } catch (RuntimeException var12_1) {
                     FontManager.LOGGER.warn("Unable to load font '{}' in fonts.json in resourcepack: '{}': {}", var8, var11.getSourceName(), var12_1.getMessage());
                  }

                  var2.pop();
               }
            } catch (IOException var10_1) {
               FontManager.LOGGER.warn("Unable to load font '{}' in fonts.json: {}", var8, var10_1.getMessage());
            }

            var2.push("caching");
            IntSet var56 = new IntOpenHashSet();

            for(GlyphProvider var58 : var9) {
               var56.addAll(var58.getSupportedGlyphs());
            }

            var56.forEach((IntConsumer) (var1x) -> {
               if (var1x != 32) {
                  for(GlyphProvider var3_ : Lists.reverse(var9)) {
                     if (var3_.getGlyph(var1x) != null) {
                        break;
                     }
                  }
      ...
      protected void apply(Map<ResourceLocation, List<GlyphProvider>> var1, ResourceManager var2, ProfilerFiller var3) {
         ...
         var1.forEach((var1x, var2x) -> {
            FontSet var3_ = new FontSet(FontManager.this.textureManager, var1x);
            var3_.reload(Lists.reverse(var2x));
            FontManager.this.fontSets.put(var1x, var3_);
         });
         var3.pop();
         var3.endTick();
      }
   ...
   public FontManager(TextureManager var1) {
      ...
      this.missingFontSet = Util.make(new FontSet(var1, MISSING_FONT), var0 -> var0.reload(Lists.newArrayList(new GlyphProvider[]{new AllMissingGlyphProvider()})));
   }
