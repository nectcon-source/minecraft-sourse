...
public class TrueTypeGlyphProviderBuilder implements GlyphProviderBuilder {
   ...
   public GlyphProvider create(ResourceManager var1) {
      ...
      try (Resource var4 = var1.getResource(new ResourceLocation(this.location.getNamespace(), "font/" + this.location.getPath()))) {
         ...
         if (!STBTruetype.stbtt_InitFont(var2, var3)) {
            ...
            TrueTypeGlyphProvider var6 = new TrueTypeGlyphProvider(var3, var2, this.size, this.oversample, this.shiftX, this.shiftY, this.skip);
            return var6;
         }
      } catch (Exception var4_1) {
         LOGGER.error("Couldn't load truetype font {}", this.location, var4_1);
         if (var2 != null) {
            var2.free();
         }
