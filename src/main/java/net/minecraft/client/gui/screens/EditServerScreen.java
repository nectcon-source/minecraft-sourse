...
public class EditServerScreen extends Screen {
   ...
   protected void init() {
      ...
      this.serverPackButton = (Button)this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 72, 200, 20, createServerButtonText(this.serverData.getResourcePackStatus()), (var1) -> {
         this.serverData.setResourcePackStatus(ServerPackStatus.values()[(this.serverData.getResourcePackStatus().ordinal() + 1) % ServerPackStatus.values().length]);
         this.serverPackButton.setMessage(createServerButtonText(this.serverData.getResourcePackStatus()));
      }));
      this.addButton = (Button)this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 96 + 18, 200, 20, new TranslatableComponent("addServer.add"), (var1) -> this.onAdd()));
      this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 120 + 18, 200, 20, CommonComponents.GUI_CANCEL, (var1) -> this.callback.accept(false)));
      this.cleanUp();
   }
