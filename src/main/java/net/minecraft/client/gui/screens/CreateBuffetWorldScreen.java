...
public class CreateBuffetWorldScreen extends Screen {
   ...
   protected void init() {
      ...
      this.doneButton = this.addButton(new Button(this.width / 2 - 155, this.height - 28, 150, 20, CommonComponents.GUI_DONE, var1 -> {
         this.applySettings.accept(this.biome);
         this.minecraft.setScreen(this.parent);
      }));
      this.addButton(new Button(this.width / 2 + 5, this.height - 28, 150, 20, CommonComponents.GUI_CANCEL, var1 -> this.minecraft.setScreen(this.parent)));
   ...
   class BiomeList extends ObjectSelectionList<CreateBuffetWorldScreen.BiomeList.Entry> {
      private BiomeList() {
         ...
         CreateBuffetWorldScreen.this.biomes
            .entrySet()
            .stream()
            .sorted(Comparator.comparing(var0 -> var0.getKey().location().toString()))
            .forEach(var1x -> this.addEntry(new CreateBuffetWorldScreen.BiomeList.Entry(var1x.getValue())));
      }
