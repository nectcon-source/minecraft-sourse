...
public class SkinCustomizationScreen extends OptionsSubScreen {
   ...
   protected void init() {
      ...
      for(PlayerModelPart var5 : PlayerModelPart.values()) {
         this.addButton(new Button(this.width / 2 - 155 + var1 % 2 * 160, this.height / 6 + 24 * (var1 >> 1), 150, 20, this.getMessage(var5), (var2) -> {
            this.options.toggleModelPart(var5);
            var2.setMessage(this.getMessage(var5));
         }));
         ++var1;
      }
      ...
      this.addButton(new Button(this.width / 2 - 100, this.height / 6 + 24 * (var1 >> 1), 200, 20, CommonComponents.GUI_DONE, (var1x) -> this.minecraft.setScreen(this.lastScreen)));
   }
