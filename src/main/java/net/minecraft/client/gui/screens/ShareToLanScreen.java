   ...
   protected void init() {
      this.addButton(new Button(this.width / 2 - 155, this.height - 28, 150, 20, new TranslatableComponent("lanServer.start"), (var1) -> {
         ...
         if (this.minecraft.getSingleplayerServer().publishServer(GameType.byName(this.gameModeName), this.commands, var2)) {
            var3 = new TranslatableComponent("commands.publish.started", var2);
         } else {
      ...
      this.addButton(new Button(this.width / 2 + 5, this.height - 28, 150, 20, CommonComponents.GUI_CANCEL, (var1) -> this.minecraft.setScreen(this.lastScreen)));
      this.modeButton = this.addButton(new Button(this.width / 2 - 155, 100, 150, 20, TextComponent.EMPTY, (var1) -> {
      ...
      this.commandsButton = this.addButton(new Button(this.width / 2 + 5, 100, 150, 20, ALLOW_COMMANDS_LABEL, (var1) -> {
