...
public class SocialInteractionsPlayerList extends ContainerObjectSelectionList<PlayerEntry> {
   ...
   public void updatePlayerList(Collection<UUID> var1, double var2) {
      ...
      for(UUID var5 : var1) {
         ...
         if (var6 != null) {
             this.players.add(new PlayerEntry(this.minecraft, this.socialInteractionsScreen, var6.getProfile().getId(), var6.getProfile().getName(), var6::getSkinLocation));
         }
      }
   ...
   public void addPlayer(PlayerInfo var1, SocialInteractionsScreen.Page var2) {
      ...
      if ((var2 == Page.ALL || this.minecraft.getPlayerSocialManager().shouldHideMessageFrom(var3)) && (Strings.isNullOrEmpty(this.filter) || var1.getProfile().getName().toLowerCase(Locale.ROOT).contains(this.filter))) {
         PlayerEntry var6 = new PlayerEntry(this.minecraft, this.socialInteractionsScreen, var1.getProfile().getId(), var1.getProfile().getName(), var1::getSkinLocation);
         this.addEntry(var6);
