...
public class RecipeButton extends AbstractWidget {
   ...
   public void init(RecipeCollection var1, RecipeBookPage var2) {
      ...
      List<Recipe<?>> var3 = var1.getRecipes(this.book.isFiltering(this.menu));
   ...
   public void renderButton(PoseStack var1, int var2, int var3, float var4) {
      ...
      if (var8) {
         float var9 = 1.0F + 0.1F * (float)Math.sin((double)(this.animationTime / 15.0F * (float)Math.PI));
      ...
      List<Recipe<?>> var12 = this.getOrderedRecipes();
   ...
   private List<Recipe<?>> getOrderedRecipes() {
      List<Recipe<?>> var1 = this.collection.getDisplayRecipes(true);
   ...
   public List<Component> getTooltipText(Screen var1) {
      ...
      List<Component> var3 = Lists.newArrayList(var1.getTooltipFromItem(var2));
      if (this.collection.getRecipes(this.book.isFiltering(this.menu)).size() > 1) {
