...
public interface MultiLineLabel {
   ...
   static MultiLineLabel create(Font var0, FormattedText var1, int var2) {
      return createFixed(var0, var0.split(var1, var2).stream().map((var1x) -> new TextWithWidth(var1x, var0.width(var1x))).collect(ImmutableList.toImmutableList()));
   }

   static MultiLineLabel create(Font var0, FormattedText var1, int var2, int var3) {
      return createFixed(var0, var0.split(var1, var2).stream().limit((long)var3).map((var1x) -> new TextWithWidth(var1x, var0.width(var1x))).collect(ImmutableList.toImmutableList()));
   }

   static MultiLineLabel create(Font var0, Component... var1) {
      return createFixed(var0, Arrays.stream(var1).map(Component::getVisualOrderText).map((var1x) -> new TextWithWidth(var1x, var0.width(var1x))).collect(ImmutableList.toImmutableList()));
   }
