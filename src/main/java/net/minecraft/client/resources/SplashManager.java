...
public class SplashManager extends SimplePreparableReloadListener<List<String>> {
   ...
   protected List<String> prepare(ResourceManager var1, ProfilerFiller var2) {
      try (Resource var3 = Minecraft.getInstance().getResourceManager().getResource(SPLASHES_LOCATION)) {
         BufferedReader var5 = new BufferedReader(new InputStreamReader(var3.getInputStream(), StandardCharsets.UTF_8));
         Throwable var6 = null;

         Object var7;
         try {
            var7 = var5.lines().map(String::trim).filter((var0) -> var0.hashCode() != 125780783).collect(Collectors.toList());
