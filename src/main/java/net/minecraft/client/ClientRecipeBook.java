...
public class ClientRecipeBook extends RecipeBook {
   ...
   public void setupCollections(Iterable<Recipe<?>> var1) {
      Map<RecipeBookCategories, List<List<Recipe<?>>>> var2 = categorizeAndGroupRecipes(var1);
      Map<RecipeBookCategories, List<RecipeCollection>> var3 = Maps.newHashMap();
      Builder<RecipeCollection> var4xx = ImmutableList.builder();
      var2.forEach((var2x, var3x) -> var3x.stream().map(RecipeCollection::new).peek(var4xx::add));
      RecipeBookCategories.AGGREGATE_CATEGORIES
         .forEach((var1x, var2x) -> var2x.stream().flatMap(var1xx -> var3.getOrDefault(var1xx, ImmutableList.of()).stream()));
      this.collectionsByTab = ImmutableMap.copyOf(var3);
      this.allCollections = var4xx.build();
   }

   private static Map<RecipeBookCategories, List<List<Recipe<?>>>> categorizeAndGroupRecipes(Iterable<Recipe<?>> var0) {
      Map<RecipeBookCategories, List<List<Recipe<?>>>> var1 = Maps.newHashMap();
      Table<RecipeBookCategories, String, List<Recipe<?>>> var2 = HashBasedTable.create();
      ...
      for(Recipe<?> var4 : var0) {
         if (!var4.isSpecial()) {
            ...
            if (var6.isEmpty()) {
               ...
               List<Recipe<?>> var7 = var2.get(var5, var6);
               if (var7 == null) {
   ...
   private static RecipeBookCategories getCategory(Recipe<?> var0) {
      ...
      if (var1 == RecipeType.CRAFTING) {
         ...
         LOGGER.warn("Unknown recipe category: {}/{}", new Supplier[]{() -> Registry.RECIPE_TYPE.getKey(var0.getType()), var0::getId});
