...
public class TextureManager implements PreparableReloadListener, Tickable, AutoCloseable {
   ...
   private void _bind(ResourceLocation var1) {
      AbstractTexture var2 = this.byPath.get(var1);
      if (var2 == null) {
   ...
   private AbstractTexture loadTexture(ResourceLocation var1, AbstractTexture var2) {
      try {
         ...
         if (var1 != INTENTIONAL_MISSING_TEXTURE) {
            LOGGER.warn("Failed to load texture: {}", var1, var3_3);
         }

         return MissingTextureAtlasSprite.getTexture();
      } catch (Throwable var3_1) {
         CrashReport var4 = CrashReport.forThrowable(var3_1, "Registering texture");
   ...
   public CompletableFuture<Void> preload(ResourceLocation var1, Executor var2) {
      if (!this.byPath.containsKey(var1)) {
         ...
         return var3.getFuture().thenRunAsync(() -> this.register((ResourceLocation)var1, (AbstractTexture)var3), TextureManager::execute);
      } else {
   ...
   @Override
   public void tick() {
      for(Tickable var1 : this.tickableTextures) {
         var1.tick();
      }
   }
   ...
   public CompletableFuture<Void> reload(
      ...
      return CompletableFuture.allOf(TitleScreen.preloadResources(this, var5), this.preload(AbstractWidget.WIDGETS_LOCATION, var5))
         ...
         .thenAcceptAsync(var3x -> {
            ...
            Iterator<Entry<ResourceLocation, AbstractTexture>> var4x = this.byPath.entrySet().iterator();

            while(var4x.hasNext()) {
               Entry<ResourceLocation, AbstractTexture> var5x = var4x.next();
               ResourceLocation var6xx = var5x.getKey();
               AbstractTexture var7xxx = var5x.getValue();
               if (var7xxx == MissingTextureAtlasSprite.getTexture() && !var6xx.equals(MissingTextureAtlasSprite.getLocation())) {
                  var4x.remove();
               } else {
                  var7xxx.reset(this, var2, var6xx, var6);
               }
            }
