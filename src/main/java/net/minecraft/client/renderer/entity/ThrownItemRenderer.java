...
public class ThrownItemRenderer<T extends Entity & ItemSupplier> extends EntityRenderer<T> {
   ...
   public void render(T var1, float var2, float var3, PoseStack var4, MultiBufferSource var5, int var6) {
      if (var1.tickCount >= 2 || !(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(var1) < (double)12.25F)) {
         var4.pushPose();
         ...
         this.itemRenderer.renderStatic(((ItemSupplier)var1).getItem(), TransformType.GROUND, var6, OverlayTexture.NO_OVERLAY, var4, var5);
         var4.popPose();
