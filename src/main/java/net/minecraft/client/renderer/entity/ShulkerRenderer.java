...
public class ShulkerRenderer extends MobRenderer<Shulker, ShulkerModel<Shulker>> {
   ...
   public Vec3 getRenderOffset(Shulker var1, float var2) {
      ...
      if (var3 > 0 && var1.hasValidInterpolationPositions()) {
         ...
         double var6 = (double)((float)var3 - var2) / (double)6.0F;
         var6 *= var6;
   ...
   static {
      ...
      TEXTURE_LOCATION = (ResourceLocation[])Sheets.SHULKER_TEXTURE_LOCATION.stream().map((var0) -> new ResourceLocation("textures/" + var0.texture().getPath() + ".png")).toArray((var0) -> new ResourceLocation[var0]);
   }
}
