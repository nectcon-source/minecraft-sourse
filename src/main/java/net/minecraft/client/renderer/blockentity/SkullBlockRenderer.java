...
public class SkullBlockRenderer extends BlockEntityRenderer<SkullBlockEntity> {
   private static final Map<SkullBlock.Type, SkullModel> MODEL_BY_TYPE = Util.make(Maps.newHashMap(), var0 -> {
   ...
   public static void renderSkull(
      @Nullable Direction var0, float var1, SkullBlock.Type var2, @Nullable GameProfile var3, float var4, PoseStack var5, MultiBufferSource var6, int var7
   ) {
      SkullModel var8 = MODEL_BY_TYPE.get(var2);
      var5.pushPose();
      if (var0 == null) {
         var5.translate(0.5F, 0.0F, 0.5F);
      } else {
         float var9 = 0.25F;
         var5.translate((0.5F - (float)var0.getStepX() * 0.25F), 0.25F, (0.5F - (float)var0.getStepZ() * 0.25F));
      }
   ...
   private static RenderType getRenderType(SkullBlock.Type var0, @Nullable GameProfile var1) {
      ...
      if (var0 == SkullBlock.Types.PLAYER && var1 != null) {
         ...
         Map<MinecraftProfileTexture.Type, MinecraftProfileTexture> var4 = var3.getSkinManager().getInsecureSkinInformation(var1);
         return var4.containsKey(Type.SKIN) ? RenderType.entityTranslucent(var3.getSkinManager().registerTexture(var4.get(Type.SKIN), Type.SKIN)) : RenderType.entityCutoutNoCull(DefaultPlayerSkin.getDefaultSkin(Player.createPlayerUUID(var1)));
      } else {
