...
public class TheEndPortalRenderer<T extends TheEndPortalBlockEntity> extends BlockEntityRenderer<T> {
   ...
   private static final List<RenderType> RENDER_TYPES = IntStream.range(0, 16)
      .mapToObj(var0 -> RenderType.endPortal(var0 + 1))
   ...
   public void render(T var1, float var2, PoseStack var3, MultiBufferSource var4, int var5, int var6) {
      ...
      this.renderCube(var1, var10, 0.15F, var11, var4.getBuffer(RENDER_TYPES.get(0)));

      for(int var12 = 1; var12 < var9; ++var12) {
         this.renderCube(var1, var10, 2.0F / (float)(18 - var12), var11, var4.getBuffer(RENDER_TYPES.get(var12)));
      }
   }
   ...
   protected int getPasses(double var1) {
      if (var1 > (double)36864.0F) {
         return 1;
      } else if (var1 > (double)25600.0F) {
         return 3;
      } else if (var1 > (double)16384.0F) {
         return 5;
      } else if (var1 > (double)9216.0F) {
         return 7;
      } else if (var1 > (double)4096.0F) {
         return 9;
      } else if (var1 > (double)1024.0F) {
         return 11;
      } else if (var1 > (double)576.0F) {
         return 13;
      } else {
         return var1 > (double)256.0F ? 14 : 15;
      }
   }
