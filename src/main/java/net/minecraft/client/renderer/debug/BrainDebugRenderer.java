...
public class BrainDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private void doRender(double var1, double var3, double var5) {
      ...
      for(BlockPos var9 : this.pois.keySet()) {
         if (var7.closerThan(var9, (double)30.0F)) {
            highlightPoi(var9);
         }
      }

      this.pois.values().forEach((var2) -> {
         if (var7.closerThan(var2.pos, (double)30.0F)) {
            this.renderPoiInfo(var2);
         }

      });
      this.getGhostPois().forEach((var2, var3x) -> {
         if (var7.closerThan(var2, (double)30.0F)) {
   ...
   private void renderPoiInfo(BrainDebugRenderer.PoiInfo var1) {
      int var2 = 0;
      Set<String> var3 = this.getTicketHolderNames(var1);
      if (var3.size() < 4) {
      ...
      ++var2;
      Set<String> var4 = this.getPotentialTicketHolderNames(var1);
      if (var4.size() < 4) {
   ...
   private Collection<UUID> getTicketHolders(BlockPos var1) {
      return this.brainDumpsPerEntity.values().stream().filter(var1x -> var1x.hasPoi(var1)).map(BrainDebugRenderer.BrainDump::getUuid).collect(Collectors.toSet());
   }
   ...
   private Map<BlockPos, List<String>> getGhostPois() {
      ...
      for(BrainDump var3 : this.brainDumpsPerEntity.values()) {
