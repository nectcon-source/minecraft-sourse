...
public class CraftPlanksTutorialStep implements TutorialStepInstance {
   ...
   public void tick() {
      ...
      if (this.tutorial.getGameMode() != GameType.SURVIVAL) {
         ...
         if (this.timeWaiting >= 1200 && this.toast == null) {
            this.toast = new TutorialToast(Icons.WOODEN_PLANKS, CRAFT_TITLE, CRAFT_DESCRIPTION, false);
   ...
   public static boolean hasCraftedPlanksPreviously(LocalPlayer var0, Tag<Item> var1) {
      for(Item var3 : var1.getValues()) {
         if (var0.getStats().getValue(Stats.ITEM_CRAFTED.get(var3)) > 0) {
            return true;
         }
      }
