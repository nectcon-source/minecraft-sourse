...
public class QueryThreadGs4 extends GenericThread {
   ...
   private QueryThreadGs4(ServerInterface var1, int var2) {
      ...
      if (!this.serverIp.isEmpty() && !this.hostIp.equals(this.serverIp)) {
         ...
         } catch (UnknownHostException var3_1) {
            LOGGER.warn("Unable to determine local host IP, please set server-ip in server.properties", var3_1);
         }
      }
      ...
      try {
         while(this.running) {
            try {
               ...
               this.recoverSocketError(var2_1);
            }
         }
      } finally {
