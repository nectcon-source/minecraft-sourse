...
public class OldUsersConverter {
   ...
   static List<String> readOldListFormat(File var0, Map<String, String[]> var1) throws IOException {
      List<String> var2 = Files.readLines(var0, StandardCharsets.UTF_8);
      ...
      if (OLD_USERBANLIST.exists() && OLD_USERBANLIST.isFile()) {
         if (var1.getFile().exists()) {
            try {
               ...
               LOGGER.warn("Could not load existing file {}", var1.getFile().getName(), var2_1);
            }
         }
         ...
         try {
            ...
            LOGGER.warn("Could not read old user banlist to convert it!", var2_5);
            return false;
         } catch (ConversionError var2_3) {
            LOGGER.error("Conversion failed, please try again later", var2_3);
            return false;
         }
      } else {
         return true;
   ...
   public static boolean convertIpBanlist(MinecraftServer var0) {
      ...
      if (OLD_IPBANLIST.exists() && OLD_IPBANLIST.isFile()) {
         ...
         try {
            ...
            LOGGER.warn("Could not parse old ip banlist to convert it!", var2_3);
            return false;
         }
      } else {
         return true;
   ...
   public static boolean convertOpsList(final MinecraftServer var0) {
      ...
      if (OLD_OPLIST.exists() && OLD_OPLIST.isFile()) {
         ...
         try {
            ...
            LOGGER.warn("Could not read old oplist to convert it!", var2_5);
            return false;
         } catch (ConversionError var2_3) {
            LOGGER.error("Conversion failed, please try again later", var2_3);
            return false;
   ...
   public static boolean convertWhiteList(final MinecraftServer var0) {
      ...
      if (OLD_WHITELIST.exists() && OLD_WHITELIST.isFile()) {
         ...
         try {
            ...
            LOGGER.warn("Could not read old whitelist to convert it!", var2_5);
            return false;
         } catch (ConversionError var2_3) {
            LOGGER.error("Conversion failed, please try again later", var2_3);
            return false;
   ...
   public static boolean convertPlayers(final DedicatedServer var0) {
      ...
      if (var1.exists() && var1.isDirectory()) {
         ...
         try {
            ...
            return true;
         } catch (ConversionError var6_4) {
            LOGGER.error("Conversion failed, please try again later", var6_4);
            return false;
         }
      } else {
         return true;
