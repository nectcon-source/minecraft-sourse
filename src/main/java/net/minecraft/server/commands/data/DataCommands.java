...
public class DataCommands {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      LiteralArgumentBuilder<CommandSourceStack> var1 = Commands.literal("data").requires(var0x -> var0x.hasPermission(2));
                        ...
                        var1x -> var1x.executes(var1xx -> getData((CommandSourceStack)var1xx.getSource(), var3.access(var1xx)))
                              .then(
                                 ((RequiredArgumentBuilder)Commands.argument("path", NbtPathArgument.nbtPath())
                                       .executes(
                                          var1xx -> getData((CommandSourceStack)var1xx.getSource(), var3.access(var1xx), NbtPathArgument.getPath(var1xx, "path"))
                                       ))
                                    .then(
               ...
               .then(
                     ...
                     var1x -> var1x.then(
                           Commands.argument("path", NbtPathArgument.nbtPath())
                              .executes(var1xx -> removeData((CommandSourceStack)var1xx.getSource(), var3.access(var1xx), NbtPathArgument.getPath(var1xx, "path")))
                        )
                  )
               ))
            .then(
               decorateModification(
                  (var0x, var1x) -> var0x.then(
                        ...
                        .then(
                           Commands.literal("set").then(var1x.create((var0xx, var1xx, var2, var3x) -> var2.set(var1xx, ((Tag)Iterables.getLast(var3x))::copy)))
                        )
                        .then(Commands.literal("merge").then(var1x.create((var0xx, var1xx, var2, var3x) -> {
                           Collection<Tag> var4 = var2.getOrCreate(var1xx, CompoundTag::new);
                           int var5x = 0;
      ...
      for(DataProvider var3 : TARGET_PROVIDERS) {
         var3.wrap(var1, (var2) -> {
            ...
            for(DataProvider var5 : SOURCE_PROVIDERS) {
               var0.accept(var3x, (DataManipulatorDecorator)(var2x) -> var5.wrap(Commands.literal("from"), (var3x__) -> var3x__.executes((var3xx) -> {
                  List<Tag> var4 = Collections.singletonList(var5.access(var3xx).getData());
                  return manipulateData(var3xx, var3, var2x, var4);
                  ...
                  List<Tag> var6 = var5x.get(var4.getData());
                  return manipulateData(var3xx, var3, var2x, var6);
               }))));
            }

            var0.accept(var3x, (DataManipulatorDecorator)(var1_) -> (LiteralArgumentBuilder)Commands.literal("value").then(Commands.argument("value", NbtTagArgument.nbtTag()).executes((var2_) -> {
               List<Tag> var3x_ = Collections.singletonList(NbtTagArgument.getNbtTag(var2_, "value"));
               return manipulateData(var2_, var3, var1_, var3x_);
            })));
