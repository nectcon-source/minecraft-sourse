...
public class StorageDataAccessor implements DataAccessor {
   private static final SuggestionProvider<CommandSourceStack> SUGGEST_STORAGE = (var0, var1) -> SharedSuggestionProvider.suggestResource(
   ...
   public Component getPrintSuccess(Tag var1) {
      return new TranslatableComponent("commands.data.storage.query", this.id, var1.getPrettyDisplay());
   }

   @Override
   public Component getPrintSuccess(NbtPathArgument.NbtPath var1, double var2, int var4) {
      return new TranslatableComponent("commands.data.storage.get", var1, this.id, String.format(Locale.ROOT, "%.2f", var2), var4);
   }
}
