...
public class LocateCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      LiteralArgumentBuilder<CommandSourceStack> var1 = (LiteralArgumentBuilder)Commands.literal("locate").requires((var0x) -> var0x.hasPermission(2));

      for(Map.Entry<String, StructureFeature<?>> var3 : StructureFeature.STRUCTURES_REGISTRY.entrySet()) {
         var1 = (LiteralArgumentBuilder)var1.then(Commands.literal((String)var3.getKey()).executes((var1x) -> locate((CommandSourceStack)var1x.getSource(), (StructureFeature)var3.getValue())));
      }
   ...
   public static int showLocateResult(CommandSourceStack var0, String var1, BlockPos var2, BlockPos var3, String var4) {
      ...
      Component var6 = ComponentUtils.wrapInSquareBrackets(new TranslatableComponent("chat.coordinates", new Object[]{var3.getX(), "~", var3.getZ()})).withStyle((var1x) -> var1x.withColor(ChatFormatting.GREEN).withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/tp @s " + var3.getX() + " ~ " + var3.getZ())).withHoverEvent(new HoverEvent(net.minecraft.network.chat.HoverEvent.Action.SHOW_TEXT, new TranslatableComponent("chat.coordinates.tooltip"))));
