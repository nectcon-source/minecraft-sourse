...
public class ServerChunkCache extends ChunkSource {
   ...
   public ServerChunkCache(
      ...
      Supplier<DimensionDataStorage> var10
   ) {
      this.level = var1;
   ...
   public ChunkAccess getChunk(int var1, int var2, ChunkStatus var3, boolean var4) {
      if (Thread.currentThread() != this.mainThread) {
         ...
         CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> var10 = this.getChunkFutureMainThread(var1, var2, var3, var4);
   ...
   public LevelChunk getChunkNow(int var1, int var2) {
      if (Thread.currentThread() != this.mainThread) {
         ...
         if (var8 == null) {
            ...
            Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure> var9 = var8.getFutureIfPresent(ChunkStatus.FULL).getNow(null);
            if (var9 == null) {
               return null;
            } else {
   ...
   private boolean checkChunkFuture(long var1, Function<ChunkHolder, CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>>> var3) {
      ...
      if (var4 == null) {
         ...
         Either<LevelChunk, ChunkHolder.ChunkLoadingFailure> var5 = (Either)((CompletableFuture)var3.apply(var4)).getNow(ChunkHolder.UNLOADED_LEVEL_CHUNK);
   ...
   private void tickChunks() {
      ...
      if (!var6) {
         ...
         this.level.getProfiler().pop();
         List<ChunkHolder> var12 = Lists.newArrayList(this.chunkMap.getChunks());
         Collections.shuffle(var12);
         var12.forEach((var7x) -> {
            Optional<LevelChunk> var8x = ((Either)var7x.getTickingChunkFuture().getNow(ChunkHolder.UNLOADED_LEVEL_CHUNK)).left();
            if (var8x.isPresent()) {
               this.level.getProfiler().push("broadcast");
               var7x.broadcastChanges((LevelChunk)var8x.get());
               this.level.getProfiler().pop();
               Optional<LevelChunk> var9x = ((Either)var7x.getEntityTickingChunkFuture().getNow(ChunkHolder.UNLOADED_LEVEL_CHUNK)).left();
               if (var9x.isPresent()) {
                  LevelChunk var10_ = (LevelChunk)var9x.get();
                  ChunkPos var11x = var7x.getPos();
                  if (!this.chunkMap.noPlayersCloseForSpawning(var11x)) {
                     var10_.setInhabitedTime(var10_.getInhabitedTime() + var3);
                     if (var7 && (this.spawnEnemies || this.spawnFriendlies) && this.level.getWorldBorder().isWithinBounds(var10_.getPos())) {
                        NaturalSpawner.spawnForChunk(this.level, var10_, var11, this.spawnFriendlies, this.spawnEnemies, var9);
                     }

                     this.level.tickChunk(var10_, var8);
                  }
               }
            }
         });
