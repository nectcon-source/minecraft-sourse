      ...
      synchronized(this.channels) {
         ...
         this.channels.add(((ServerBootstrap)((ServerBootstrap)(new ServerBootstrap()).channel(var4)).childHandler(new ChannelInitializer<Channel>() {
            protected void initChannel(Channel var1) throws Exception {
               ...
               Connection var3 = (Connection)(var2 > 0 ? new RateKickingConnection(var2) : new Connection(PacketFlow.SERVERBOUND));
               ServerConnectionListener.this.connections.add(var3);
               var1.pipeline().addLast("packet_handler", var3);
               var3.setListener(new ServerHandshakePacketListenerImpl(ServerConnectionListener.this.server, var3));
            }
               ...
               if (var3.isConnected()) {
                  try {
                     ...
                     if (var3.isMemoryConnection()) {
                        throw new ReportedException(CrashReport.forThrowable(var4_1, "Ticking memory connection"));
                     }

                     LOGGER.warn("Failed to handle packet for {}", var3.getRemoteAddress(), var4_1);
