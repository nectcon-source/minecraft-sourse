...
/* loaded from: client_deobf_norm.jar:net/minecraft/world/level/levelgen/feature/Feature.class */
public abstract class Feature<FC extends FeatureConfiguration> {
    ...
    public static final AbstractFlowerFeature<RandomPatchConfiguration> FLOWER = (AbstractFlowerFeature) register("flower", new DefaultFlowerFeature(RandomPatchConfiguration.CODEC));
    public static final AbstractFlowerFeature<RandomPatchConfiguration> NO_BONEMEAL_FLOWER = (AbstractFlowerFeature) register("no_bonemeal_flower", new DefaultFlowerFeature(RandomPatchConfiguration.CODEC));
    ...
    private Codec<ConfiguredFeature<FC, Feature<FC>>> configuredCodec = null;

    public abstract boolean place(WorldGenLevel worldGenLevel, ChunkGenerator chunkGenerator, Random random, BlockPos blockPos, FC fc);

    private static <C extends FeatureConfiguration, F extends Feature<C>> F register(String str, F f) {
        return (F) Registry.register(Registry.FEATURE, str, f);
    }

    public Feature(Codec<FC> codec) {
//        (Codec<ConfiguredFeature<FC, Feature<FC>>>)this.configuredCodec = codec.fieldOf("config").xmap((var1x) -> new ConfiguredFeature(this, var1x), (var0) -> (FC) var0.config).codec();
        this.configuredCodec = codec.fieldOf("config")
                .xmap(
                        (var1x) -> new ConfiguredFeature<>(this, var1x),
                        (var0) -> (FC) var0.config
                )
                .codec();
    }
