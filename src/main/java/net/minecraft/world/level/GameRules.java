...
public class GameRules {
    ...
    public static final GameRules.Key<GameRules.BooleanValue> RULE_REDUCEDDEBUGINFO = register(
            "reducedDebugInfo", GameRules.Category.MISC, GameRules.BooleanValue.create(false, (var0, var1) -> {
                byte var5 = (byte)(var1.get() ? 22 : 23);

                for(ServerPlayer var6 : var0.getPlayerList().getPlayers()) {
            var6.connection.send(new ClientboundEntityEventPacket(var6, var5));
                }
            })
    );
    ...
    public static final GameRules.Key<GameRules.BooleanValue> RULE_DO_IMMEDIATE_RESPAWN = register(
            ...
            var2.connection.send(new ClientboundGameEventPacket(ClientboundGameEventPacket.IMMEDIATE_RESPAWN, var1.get() ? 1.0F : 0.0F));
                }
            })
    );
    ...
    private static <T extends GameRules.Value<T>> GameRules.Key<T> register(String var0, GameRules.Category var1, GameRules.Type<T> var2) {
        Key<T> var3 = new Key<T>(var0, var1);
    ...
    public GameRules() {
        this.rules = (Map)GAME_RULE_TYPES.entrySet().stream().collect(ImmutableMap.toImmutableMap(Map.Entry::getKey, (var0) -> ((Type)var0.getValue()).createRule()));
    }
    ...
    public CompoundTag createTag() {
        CompoundTag varr = new CompoundTag();
        this.rules.forEach((var1x, var2) -> varr.putString(var1x.id, var2.serialize()));
        return varr;
    }
    ...
    public static class BooleanValue extends GameRules.Value<GameRules.BooleanValue> {
        ...
        private static GameRules.Type<GameRules.BooleanValue> create(boolean var0, BiConsumer<MinecraftServer, GameRules.BooleanValue> var1) {
            return new GameRules.Type<>(BoolArgumentType::bool, var1x -> new GameRules.BooleanValue(var1x, var0), var1, GameRules.GameRuleTypeVisitor::visitBoolean);
        }
...
//        }
        ...
        default <T extends Value<T>> void visit(Key<?> var1, Type<?> var2) {
        }
    }
    ...
    public static class Type<T extends GameRules.Value<T>> {
        ...
        public void callVisitor(GameRuleTypeVisitor var1, Key<?> var2) {
            this.visitorCaller.call(var1, (Key<T>) var2, this);
        }
    }
