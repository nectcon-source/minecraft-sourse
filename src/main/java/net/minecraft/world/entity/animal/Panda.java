...
public class Panda extends Animal {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, (double)1.0F, Ingredient.of(new ItemLike[]{Blocks.BAMBOO.asItem()}), false));
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.MOVEMENT_SPEED, (double)0.15F).add(Attributes.ATTACK_DAMAGE, (double)6.0F);
    }
    ...
    private void addEatingParticles() {
        if (this.getEatCounter() % 5 == 0) {
            ...
            for(int var1 = 0; var1 < 6; ++var1) {
                Vec3 var2 = new Vec3(((double)this.random.nextFloat() - (double)0.5F) * 0.1, Math.random() * 0.1 + 0.1, ((double)this.random.nextFloat() - (double)0.5F) * 0.1);
                var2 = var2.xRot(-this.xRot * ((float)Math.PI / 180F));
                var2 = var2.yRot(-this.yRot * ((float)Math.PI / 180F));
                double var3 = (double)(-this.random.nextFloat()) * 0.6 - 0.3;
                Vec3 var5 = new Vec3(((double)this.random.nextFloat() - (double)0.5F) * 0.8, var3, (double)1.0F + ((double)this.random.nextFloat() - (double)0.5F) * 0.4);
                var5 = var5.yRot(-this.yBodyRot * ((float)Math.PI / 180F));
                var5 = var5.add(this.getX(), this.getEyeY() + (double)1.0F, this.getZ());
        ...
        if (this.rollCounter > 32) {
            ...
            if (!this.level.isClientSide) {
                ...
                if (this.rollCounter == 1) {
                    float var2 = this.yRot * ((float)Math.PI / 180F);
                    float var3 = this.isBaby() ? 0.1F : 0.2F;
                    this.rollDelta = new Vec3(var1.x + (double)(-Mth.sin(var2) * var3), (double)0.0F, var1.z + (double)(Mth.cos(var2) * var3));
                    ...
                    this.setDeltaMovement((double)0.0F, this.onGround ? 0.27 : var1.y, (double)0.0F);
                }
            }
    ...
    private void afterSneeze() {
        Vec3 var1 = this.getDeltaMovement();
        this.level.addParticle(ParticleTypes.SNEEZE, this.getX() - (double)(this.getBbWidth() + 1.0F) * (double)0.5F * (double)Mth.sin(this.yBodyRot * ((float)Math.PI / 180F)), this.getEyeY() - (double)0.1F, this.getZ() + (double)(this.getBbWidth() + 1.0F) * (double)0.5F * (double)Mth.cos(this.yBodyRot * ((float)Math.PI / 180F)), var1.x, (double)0.0F, var1.z);
        this.playSound(SoundEvents.PANDA_SNEEZE, 1.0F, 1.0F);

        for(Panda var4 : this.level.getEntitiesOfClass(Panda.class, this.getBoundingBox().inflate((double)10.0F))) {
    ...
    static {
        ...
        BREED_TARGETING = (new TargetingConditions()).range((double)8.0F).allowSameTeam().allowInvulnerable();
        PANDA_ITEMS = (var0) -> {
    ...
    public static enum Gene {
        ...
        private static final Gene[] BY_ID = (Gene[])Arrays.stream(values()).sorted(Comparator.comparingInt(Gene::getId)).toArray((var0) -> new Gene[var0]);
        ...
        private static Gene getVariantFromGenes(Gene var0, Gene var1) {
            if (var0.isRecessive()) {
        ...
        public static Gene getRandom(Random var0) {
            ...
            if (this.mob.getRandom().nextFloat() >= this.probability) {
                ...
                if (this.lookAt == null) {
                    if (this.lookAtType == Player.class) {
                        ...
                        this.lookAt = this.mob.level.getNearestLoadedEntity(this.lookAtType, this.lookAtContext, this.mob, this.mob.getX(), this.mob.getEyeY(), this.mob.getZ(), this.mob.getBoundingBox().inflate((double)this.lookDistance, (double)3.0F, (double)this.lookDistance));
                    }
                }
        ...
        public boolean canUse() {
            if ((this.panda.isBaby() || this.panda.isPlayful()) && this.panda.onGround) {
                if (!this.panda.canPerformAction()) {
                    ...
                    float var1 = this.panda.yRot * ((float)Math.PI / 180F);
                    int var2 = 0;
                    int var3 = 0;
                    float var4 = -Mth.sin(var1);
                    float var5 = Mth.cos(var1);
                    if ((double)Math.abs(var4) > (double)0.5F) {
                        var2 = (int)((float)var2 + var4 / Math.abs(var4));
                    }

                    if ((double)Math.abs(var5) > (double)0.5F) {
    ...
    class PandaSitGoal extends Goal {
        ...
        public boolean canUse() {
            if (this.cooldown <= Panda.this.tickCount && !Panda.this.isBaby() && !Panda.this.isInWater() && Panda.this.canPerformAction() && Panda.this.getUnhappyCounter() <= 0) {
                List<ItemEntity> var1 = Panda.this.level.getEntitiesOfClass(ItemEntity.class, Panda.this.getBoundingBox().inflate((double)6.0F, (double)6.0F, (double)6.0F), Panda.PANDA_ITEMS);
        ...
        public void start() {
            List<ItemEntity> var1 = Panda.this.level.getEntitiesOfClass(ItemEntity.class, Panda.this.getBoundingBox().inflate((double)8.0F, (double)8.0F, (double)8.0F), Panda.PANDA_ITEMS);
            if (!var1.isEmpty() && Panda.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                Panda.this.getNavigation().moveTo((Entity)var1.get(0), (double)1.2F);
