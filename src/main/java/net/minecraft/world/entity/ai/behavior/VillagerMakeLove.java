...
public class VillagerMakeLove extends Behavior<Villager> {
   ...
   protected void tick(ServerLevel var1, Villager var2, long var3) {
      ...
      if (!(var2.distanceToSqr(var5) > (double)5.0F)) {
         BehaviorUtils.lockGazeAndWalkToEachOther(var2, var5, 0.5F);
   ...
   private void tryToGiveBirth(ServerLevel var1, Villager var2, Villager var3) {
      Optional<BlockPos> var4 = this.takeVacantBed(var1, var2);
      if (!var4.isPresent()) {
         var1.broadcastEntityEvent(var3, (byte)13);
         var1.broadcastEntityEvent(var2, (byte)13);
      } else {
         Optional<Villager> var5 = this.breed(var1, var2, var3);
         if (var5.isPresent()) {
   ...
   private boolean isBreedingPossible(Villager var1) {
      ...
      Optional<AgableMob> var3 = var2.getMemory(MemoryModuleType.BREED_TARGET).filter((var0) -> var0.getType() == EntityType.VILLAGER);
      if (!var3.isPresent()) {
         return false;
      } else {
   ...
   private Optional<BlockPos> takeVacantBed(ServerLevel var1, Villager var2) {
      return var1.getPoiManager().take(PoiType.HOME.getPredicate(), (var2x) -> this.canReach(var2, var2x), var2.blockPosition(), 48);
   }
