...
public class BehaviorUtils {
   ...
   public static boolean entityIsVisible(Brain<?> var0, LivingEntity var1) {
      return var0.getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES).filter((var1x) -> var1x.contains(var1)).isPresent();
   }
   ...
   public static SectionPos findSectionClosestToVillage(ServerLevel var0, SectionPos var1, int var2) {
      ...
      return SectionPos.cube(var1, var2).filter(var2x -> var0.sectionsToVillage(var2x) < var3).min(Comparator.comparingInt(var0::sectionsToVillage)).orElse(var1);
   }
   ...
   public static boolean isOtherTargetMuchFurtherAwayThanCurrentAttackTarget(LivingEntity var0, LivingEntity var1, double var2) {
      Optional<LivingEntity> var4 = var0.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET);
      if (!var4.isPresent()) {
         return false;
      } else {
   ...
   public static Optional<LivingEntity> getLivingEntityFromUUIDMemory(LivingEntity var0, MemoryModuleType<UUID> var1) {
      Optional<UUID> var2 = var0.getBrain().getMemory(var1);
      return var2.map((var1x) -> (LivingEntity)((ServerLevel)var0.level).getEntity(var1x));
   }
