...
public class AcquirePoi extends Behavior<PathfinderMob> {
   ...
   private static ImmutableMap<MemoryModuleType<?>, MemoryStatus> constructEntryConditionMap(MemoryModuleType<GlobalPos> var0, MemoryModuleType<GlobalPos> var1) {
      ImmutableMap.Builder<MemoryModuleType<?>, MemoryStatus> var2 = ImmutableMap.builder();
   ...
   protected void start(ServerLevel var1, PathfinderMob var2, long var3) {
      ...
      this.batchCache.long2ObjectEntrySet().removeIf((var2x) -> !((JitteredLinearRetry)var2x.getValue()).isStillValid(var3));
      Predicate<BlockPos> var6 = (var3x) -> {
      ...
      Set<BlockPos> var7 = var5.findAllClosestFirst(this.poiType.getPredicate(), var6, var2.blockPosition(), 48, PoiManager.Occupancy.HAS_SPACE).limit(5L).collect(Collectors.toSet());
      ...
      if (var8 != null && var8.canReach()) {
         ...
         var5.getType(var11).ifPresent((var5x) -> {
            var5.take(this.poiType.getPredicate(), (var1x) -> var1x.equals(var11), var11, 1);
            var2.getBrain().setMemory(this.memoryToAcquire, GlobalPos.of(var1.dimension(), var11));
            this.onPoiAcquisitionEvent.ifPresent((var2x) -> var1.broadcastEntityEvent(var2, var2x));
            this.batchCache.clear();
            DebugPackets.sendPoiTicketCountPacket(var1, var11);
         });
      } else {
         for(BlockPos var10 : var7) {
            this.batchCache.computeIfAbsent(var10.asLong(), (var3x) -> new JitteredLinearRetry(var2.level.random, var3));
         }
      }
   }
