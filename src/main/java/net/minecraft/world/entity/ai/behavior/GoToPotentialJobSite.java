...
public class GoToPotentialJobSite extends Behavior<Villager> {
   ...
   protected void tick(ServerLevel var1, Villager var2, long var3) {
      BehaviorUtils.setWalkAndLookTargetMemories(var2, ((GlobalPos)var2.getBrain().getMemory(MemoryModuleType.POTENTIAL_JOB_SITE).get()).pos(), this.speedModifier, 1);
   }

   protected void stop(ServerLevel var1, Villager var2, long var3) {
      Optional<GlobalPos> var5 = var2.getBrain().getMemory(MemoryModuleType.POTENTIAL_JOB_SITE);
      var5.ifPresent((var1x) -> {
         BlockPos var2_ = var1x.pos();
         ServerLevel var3_ = var1.getServer().getLevel(var1x.dimension());
         if (var3_ != null) {
            PoiManager var4 = var3_.getPoiManager();
            if (var4.exists(var2_, (var0) -> true)) {
               var4.release(var2_);
            }

            DebugPackets.sendPoiTicketCountPacket(var1, var2_);
         }
      });
