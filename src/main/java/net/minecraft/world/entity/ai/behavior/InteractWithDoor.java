...
public class InteractWithDoor extends Behavior<LivingEntity> {
   ...
   public static void closeDoorsThatIHaveOpenedOrPassedThrough(ServerLevel var0, LivingEntity var1, @Nullable Node var2, @Nullable Node var3) {
      ...
      if (var4.hasMemoryValue(MemoryModuleType.DOORS_TO_CLOSE)) {
         Iterator<GlobalPos> var5 = ((Set)var4.getMemory(MemoryModuleType.DOORS_TO_CLOSE).get()).iterator();
   ...
   private static boolean areOtherMobsComingThroughDoor(ServerLevel var0, LivingEntity var1, BlockPos var2) {
      ...
      return !var3.hasMemoryValue(MemoryModuleType.LIVING_ENTITIES) ? false : (var3.getMemory(MemoryModuleType.LIVING_ENTITIES).get()).stream().filter((var1x) -> var1x.getType() == var1.getType()).filter((var1x) -> var2.closerThan(var1x.position(), (double)2.0F)).anyMatch((var2x) -> isMobComingThroughDoor(var0, var2x, var2));
   }
   ...
   private static boolean isDoorTooFarAway(ServerLevel var0, LivingEntity var1, GlobalPos var2) {
      return var2.dimension() != var0.dimension() || !var2.pos().closerThan(var1.position(), (double)2.0F);
   }
