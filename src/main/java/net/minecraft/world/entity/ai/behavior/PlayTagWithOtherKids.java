...
public class PlayTagWithOtherKids extends Behavior<PathfinderMob> {
   ...
   protected void start(ServerLevel var1, PathfinderMob var2, long var3) {
      ...
      if (var5 != null) {
         ...
         Optional<LivingEntity> var6 = this.findSomeoneBeingChased(var2);
         if (var6.isPresent()) {
   ...
   private Optional<LivingEntity> findSomeoneBeingChased(PathfinderMob var1) {
      Map<LivingEntity, Integer> var2 = this.checkHowManyChasersEachFriendHas(var1);
      return var2.entrySet().stream().sorted(Comparator.comparingInt(Map.Entry::getValue)).filter((var0) -> var0.getValue() > 0 && (Integer)var0.getValue() <= 5).map(Map.Entry::getKey).findFirst();
   }

   private Map<LivingEntity, Integer> checkHowManyChasersEachFriendHas(PathfinderMob var1) {
      Map<LivingEntity, Integer> var2 = Maps.newHashMap();
      this.getFriendsNearby(var1).stream().filter(this::isChasingSomeone).forEach((var2x) -> {
         var2.compute(this.whoAreYouChasing(var2x), (var0, var1_) -> var1_ == null ? 1 : var1_ + 1);
      });
      return var2;
   }
