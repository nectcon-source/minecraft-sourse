...
public class Brain<E extends LivingEntity> {
    ...
    public static <E extends LivingEntity> Codec<Brain<E>> codec(
        ...
        final MutableObject<Codec<Brain<E>>> var2 = new MutableObject();
        var2.setValue(
                (new MapCodec<Brain<E>>() {
                    public <T> Stream<T> keys(DynamicOps<T> var1x) {
                        return var0.stream()
                                .flatMap(var0xx -> Util.toStream(var0xx.getCodec().map(var1xxxx -> Registry.MEMORY_MODULE_TYPE.getKey(var0xx))))
                                .map(var1xxx -> (T)var1x.createString(var1xxx.toString()));
                    }

                    public <T> DataResult<Brain<E>> decode(DynamicOps<T> var1x, MapLike<T> var2x) {
                        MutableObject<DataResult<Builder<Brain.MemoryValue<?>>>> var3 = new MutableObject(DataResult.success(ImmutableList.builder()));
                        var2x.entries().forEach(var3x -> {
                            DataResult<MemoryModuleType<?>> var4x = Registry.MEMORY_MODULE_TYPE.parse(var1x, var3x.getFirst());
                            DataResult<? extends Brain.MemoryValue<?>> var5x = var4x.flatMap(var3xx -> this.captureRead(var3xx, var1x, var3x.getSecond()));
                            var3.setValue((var3.getValue()).apply2(Builder::add, var5x));
                        });
                        ImmutableList<Brain.MemoryValue<?>> var4 = (var3.getValue())
                    ...
                    private <T, U> DataResult<Brain.MemoryValue<U>> captureRead(MemoryModuleType<U> var1x, DynamicOps<T> var2x, T var3) {
                        return (var1x.getCodec().map(DataResult::success).orElseGet(() -> DataResult.error("No codec for memory: " + var1x)))
    ...
    public Brain(
        ...
        for(SensorType<? extends Sensor<? super E>> var12 : var2) {
            this.sensors.put(var12, var12.create());
        }

        for(Sensor<? super E> var13 : this.sensors.values()) {
    ...
    public <U> Optional<U> getMemory(MemoryModuleType<U> var1) {
        return (Optional<U>) this.memories.get(var1).map(ExpirableValue::getValue);
    }
    ...
    public boolean checkMemory(MemoryModuleType<?> var1, MemoryStatus var2) {
        Optional<? extends ExpirableValue<?>> var3 = (Optional)this.memories.get(var1);
        if (var3 == null) {
            return false;
        } else {
    ...
    public List<Behavior<? super E>> getRunningBehaviors() {
        List<Behavior<? super E>> var1 = new ObjectArrayList();

        for(Map<Activity, Set<Behavior<? super E>>> var3 : this.availableBehaviorsByPriority.values()) {
            for(Set<Behavior<? super E>> var5 : var3.values()) {
                for(Behavior<? super E> var7 : var5) {
    ...
    private void eraseMemoriesForOtherActivitesThan(Activity var1) {
        for(Activity var3 : this.activeActivities) {
            if (var3 != var1) {
                Set<MemoryModuleType<?>> var4 = (Set)this.activityMemoriesToEraseWhenStopped.get(var3);
                if (var4 != null) {
    ...
    public void addActivityAndRemoveMemoryWhenStopped(Activity var1, int var2, ImmutableList<? extends Behavior<? super E>> var3, MemoryModuleType<?> var4) {
        Set<Pair<MemoryModuleType<?>, MemoryStatus>> var5 = ImmutableSet.of(Pair.of(var4, MemoryStatus.VALUE_PRESENT));
        Set<MemoryModuleType<?>> var6 = ImmutableSet.of(var4);
    ...
    private void addActivityAndRemoveMemoriesWhenStopped(
        ...
        while(var5.hasNext()) {
            Pair<Integer, ? extends Behavior<? super E>> var6 = (Pair)var5.next();
            ((this.availableBehaviorsByPriority.computeIfAbsent(var6.getFirst(), (var0) -> Maps.newHashMap())).computeIfAbsent(var1, (var0) -> Sets.newLinkedHashSet())).add(var6.getSecond());
        }
    }
    ...
    public Brain<E> copyWithoutBehaviors() {
        Brain<E> var1 = new Brain<E>(this.memories.keySet(), this.sensors.keySet(), ImmutableList.of(), this.codec);

        for(Map.Entry<MemoryModuleType<?>, Optional<? extends ExpirableValue<?>>> var3 : this.memories.entrySet()) {
    ...
    private void tickSensors(ServerLevel var1, E var2) {
        for(Sensor<? super E> var4 : this.sensors.values()) {
            var4.tick(var1, var2);
        }
    }

    private void forgetOutdatedMemories() {
        for(Map.Entry<MemoryModuleType<?>, Optional<? extends ExpirableValue<?>>> var2 : this.memories.entrySet()) {
    ...
    public void stopAll(ServerLevel var1, E var2) {
        ...
        for(Behavior<? super E> var6 : this.getRunningBehaviors()) {
    ...
    private void startEachNonRunningBehavior(ServerLevel var1, E var2) {
        ...
        for(Map<Activity, Set<Behavior<? super E>>> var6 : this.availableBehaviorsByPriority.values()) {
            for(Map.Entry<Activity, Set<Behavior<? super E>>> var8 : var6.entrySet()) {
                Activity var9 = var8.getKey();
                if (this.activeActivities.contains(var9)) {
                    for(Behavior<? super E> var12 : var8.getValue()) {
    ...
    private boolean activityRequirementsAreMet(Activity var1) {
        if (!this.activityRequirements.containsKey(var1)) {
            ...
            for(Pair<MemoryModuleType<?>, MemoryStatus> var3 : this.activityRequirements.get(var1)) {
                MemoryModuleType<?> var4 = var3.getFirst();
                MemoryStatus var5 = var3.getSecond();
    ...
    ImmutableList<? extends Pair<Integer, ? extends Behavior<? super E>>> createPriorityPairs(int var1, ImmutableList<? extends Behavior<? super E>> var2) {
        int var3 = var1;
        ImmutableList.Builder<Pair<Integer, ? extends Behavior<? super E>>> var4 = ImmutableList.builder();
        UnmodifiableIterator var5 = var2.iterator();

        while(var5.hasNext()) {
            Behavior<? super E> var6 = (Behavior)var5.next();
    ...
    static final class MemoryValue<U> {
        ...
        private static <U> Brain.MemoryValue<U> createUnchecked(MemoryModuleType<U> var0, Optional<? extends ExpirableValue<?>> var1) {
            return new MemoryValue<U>(var0, (Optional<? extends ExpirableValue<U>>) var1);
        }
        ...
        public <T> void serialize(DynamicOps<T> var1, RecordBuilder<T> var2) {
            this.type
                    .getCodec()
                    .ifPresent((var3) -> this.value.ifPresent((var4) -> var2.add(Registry.MEMORY_MODULE_TYPE.encodeStart(var1, this.type), var3.encodeStart(var1, var4))));
        }
    }
