...
public class SleepInBed extends Behavior<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel var1, LivingEntity var2) {
      if (var2.isPassenger()) {
         ...
         if (var1.dimension() != var4.dimension()) {
            ...
            Optional<Long> var5 = var3.getMemory(MemoryModuleType.LAST_WOKEN);
            if (var5.isPresent()) {
            ...
            return var4.pos().closerThan(var2.position(), (double)2.0F) && var8.getBlock().is(BlockTags.BEDS) && !(Boolean)var8.getValue(BedBlock.OCCUPIED);
         }
      }
   }
   ...
   protected boolean canStillUse(ServerLevel var1, LivingEntity var2, long var3) {
      Optional<GlobalPos> var5 = var2.getBrain().getMemory(MemoryModuleType.HOME);
      if (!var5.isPresent()) {
         return false;
      } else {
         BlockPos var6 = ((GlobalPos)var5.get()).pos();
         return var2.getBrain().isActive(Activity.REST) && var2.getY() > (double)var6.getY() + 0.4 && var6.closerThan(var2.position(), 1.14);
      }
   }
