...
//        Stream var10000 = Registry.ENTITY_TYPE.stream().filter((var0) -> var0.getCategory() != MobCategory.MISC).filter((var0) -> !hasSupplier(var0));
...
//            }
//        });
        // 1. Получаем поток (Stream) всех типов сущностей (EntityType), исключая:
        //    - Сущности категории MISC (незначительные, например, стрелы)
        //    - Сущности, у которых нет поставщика атрибутов (hasSupplier)
        Registry.ENTITY_TYPE.stream()
                ...
                // 2. Преобразуем EntityType в их ID (имена, например, "minecraft:zombie")
                .map(Registry.ENTITY_TYPE::getKey)

                // 3. Для каждого ID проверяем наличие атрибутов
                .forEach(entityId -> {
                    if (SharedConstants.IS_RUNNING_IN_IDE) {
                        // В режиме разработки бросаем исключение
                        throw new IllegalStateException("Entity " + entityId + " has no attributes");
                    } else {
                        // В продакшене просто пишем в лог
