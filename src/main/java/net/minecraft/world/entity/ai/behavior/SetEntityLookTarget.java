...
public class SetEntityLookTarget extends Behavior<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel var1, LivingEntity var2) {
      return (var2.getBrain().getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES).get()).stream().anyMatch(this.predicate);
   }
      ...
      var5.getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES)
         .ifPresent(
            var3x -> var3x.stream()
                  ...
                  .filter(var2xx -> var2xx.distanceToSqr(var2) <= (double)this.maxDistSqr)
                  .findFirst()
                  .ifPresent(var1xx -> var5.setMemory(MemoryModuleType.LOOK_TARGET, new EntityTracker(var1xx, true)))
         );
   }
}
