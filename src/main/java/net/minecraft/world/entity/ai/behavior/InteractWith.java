...
public class InteractWith<E extends LivingEntity, T extends LivingEntity> extends Behavior<E> {
   ...
   public static <T extends LivingEntity> InteractWith<LivingEntity, T> of(
      ...
      return new InteractWith<LivingEntity, T>(var0, var1, (var0x) -> true, (var0x) -> true, var2, var3, var4);
   }
   ...
   private boolean seesAtLeastOneValidTarget(E var1) {
      List<LivingEntity> var2 = (List)var1.getBrain().getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES).get();
   ...
   protected void start(ServerLevel var1, E var2, long var3) {
      ...
      var5.getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES)
         .ifPresent(
            var3x -> var3x.stream()
                  .filter(var1xx -> this.type.equals(var1xx.getType()))
                  .map(var0 -> var0)
                  .filter(var2xx -> var2xx.distanceToSqr(var2) <= (double)this.interactionRangeSqr)
                    .filter(entity -> this.targetFilter.test((T)entity))
                  .findFirst()
                  .ifPresent(var2xx -> {
                     var5.setMemory(this.memory, (T)var2xx);
                     var5.setMemory(MemoryModuleType.LOOK_TARGET, new EntityTracker(var2xx, true));
                     var5.setMemory(MemoryModuleType.WALK_TARGET, new WalkTarget(new EntityTracker(var2xx, false), this.speedModifier, this.maxDist));
                  })
         );
   }
}
