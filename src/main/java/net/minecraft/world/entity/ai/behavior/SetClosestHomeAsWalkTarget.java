...
public class SetClosestHomeAsWalkTarget extends Behavior<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel var1, LivingEntity var2) {
      if (var1.getGameTime() - this.lastUpdate < 20L) {
         ...
         Optional<BlockPos> var5 = var4.findClosest(PoiType.HOME.getPredicate(), var2.blockPosition(), 48, PoiManager.Occupancy.ANY);
         return var5.isPresent() && !(((BlockPos)var5.get()).distSqr(var3.blockPosition()) <= (double)4.0F);
      }
   }
   ...
   protected void start(ServerLevel var1, LivingEntity var2, long var3) {
      ...
      Predicate<BlockPos> var7 = (var1x) -> {
         long var2_ = var1x.asLong();
         if (this.batchCache.containsKey(var2_)) {
            return false;
         } else if (++this.triedCount >= 5) {
            return false;
         } else {
            this.batchCache.put(var2_, this.lastUpdate + 40L);
            return true;
         }
      };
      Stream<BlockPos> var8 = var6.findAll(PoiType.HOME.getPredicate(), var7, var2.blockPosition(), 48, PoiManager.Occupancy.ANY);
