...
public class GameTestRunner {
   ...
   public static Collection<GameTestBatch> groupTestsIntoBatches(Collection<TestFunction> var0) {
      Map<String, Collection<TestFunction>> var1 = Maps.newHashMap();
      var0.forEach((var1x) -> {
         String var2 = var1x.getBatchName();
         Collection<TestFunction> var3 = var1.computeIfAbsent(var2, (var0x) -> Lists.newArrayList());
         var3.add(var1x);
      });
      return var1.keySet().stream().flatMap((var1x) -> {
         Collection<TestFunction> var2 = var1.get(var1x);
         Consumer<ServerLevel> var3 = GameTestRegistry.getBeforeBatchFunction(var1x);
         MutableInt var4 = new MutableInt();
         return Streams.stream(Iterables.partition(var2, 100)).map((var4x) -> new GameTestBatch(var1x + ":" + var4.incrementAndGet(), var2, var3));
   ...
   private static void say(ServerLevel var0, ChatFormatting var1, String var2) {
      var0.getPlayers((var0x) -> true).forEach((var2x) -> var2x.sendMessage((new TextComponent(var2)).withStyle(var1), Util.NIL_UUID));
   }
   ...
   public static void clearAllTests(ServerLevel var0, BlockPos var1, GameTestTicker var2, int var3) {
      ...
      BlockPos var5x = var1.offset(var3, 0, var3);
      BlockPos.betweenClosedStream(var4, var5x).filter(var1x -> var0.getBlockState(var1x).is(Blocks.STRUCTURE_BLOCK)).forEach(var1x -> {
         StructureBlockEntity var2x = (StructureBlockEntity)var0.getBlockEntity(var1x);
         BlockPos var3x = var2x.getBlockPos();
         BoundingBox var4xx = StructureUtils.getStructureBoundingBox(var2x);
         StructureUtils.clearSpaceForStructure(var4xx, var3x.getY(), var0);
      });
   }
}
