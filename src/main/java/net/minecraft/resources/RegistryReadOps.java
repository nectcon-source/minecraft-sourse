...
public class RegistryReadOps<T> extends DelegatingOps<T> {
   ...
   private RegistryReadOps(
      ...
      this.readCache = var4;
      this.jsonOps = var1 == JsonOps.INSTANCE ? (RegistryReadOps<JsonElement>) this : new RegistryReadOps(JsonOps.INSTANCE, var2, var3, var4);
   }
   ...
   protected <E> DataResult<Pair<Supplier<E>, T>> decodeElement(T var1, ResourceKey<? extends Registry<E>> var2, Codec<E> var3, boolean var4) {
      Optional<WritableRegistry<E>> var5 = this.registryHolder.registry(var2);
      if (!var5.isPresent()) {
         return DataResult.error("Unknown registry: " + var2);
      } else {
         WritableRegistry<E> var6 = (WritableRegistry)var5.get();
         DataResult<Pair<ResourceLocation, T>> var7 = ResourceLocation.CODEC.decode(this.delegate, var1);
         if (!var7.result().isPresent()) {
            return !var4 ? DataResult.error("Inline definitions not allowed here") : var3.decode(this, var1).map((var0) -> var0.mapFirst((var0x) -> () -> var0x));
         } else {
            Pair<ResourceLocation, T> var8 = (Pair)var7.result().get();
            ResourceLocation var9 = (ResourceLocation)var8.getFirst();
            return this.readAndRegisterElement(var2, var6, var3, var9).map((var1x) -> Pair.of(var1x, var8.getSecond()));
         }
      }
   }
   ...
   public <E> DataResult<MappedRegistry<E>> decodeElements(MappedRegistry<E> var1, ResourceKey<? extends Registry<E>> var2, Codec<E> var3) {
      Collection<ResourceLocation> var4 = this.resources.listResources(var2);
      DataResult<MappedRegistry<E>> var5 = DataResult.success(var1, Lifecycle.stable());
      String var6 = var2.location().getPath() + "/";

      for(ResourceLocation var8 : var4) {
         ...
         if (!var9.endsWith(".json")) {
            ...
            var5 = var5.flatMap((var4x) -> this.readAndRegisterElement(var2, var4x, var3, var11).map((var1_) -> var4x));
         }
      }
   ...
   private <E> DataResult<Supplier<E>> readAndRegisterElement(
      ResourceKey<? extends Registry<E>> var1, WritableRegistry<E> var2, Codec<E> var3, ResourceLocation var4
   ) {
      ResourceKey<E> var5 = ResourceKey.create(var1, var4);
      ReadCache<E> var6 = this.<E>readCache(var1);
      DataResult<Supplier<E>> var7 = (DataResult)var6.values.get(var5);
      if (var7 != null) {
         return var7;
      } else {
         Supplier<E> var8 = Suppliers.memoize(() -> {
            E var2x = (E)var2.get(var5);
            if (var2x == null) {
         ...
         DataResult<Pair<E, OptionalInt>> var9 = this.resources.parseElement(this.jsonOps, var1, var5, var3);
         Optional<Pair<E, OptionalInt>> var10 = var9.result();
         if (var10.isPresent()) {
            Pair<E, OptionalInt> var11 = (Pair)var10.get();
            var2.registerOrOverride((OptionalInt)var11.getSecond(), var5, var11.getFirst(), var9.lifecycle());
         }

         DataResult<Supplier<E>> var12;
         if (!var10.isPresent() && var2.get(var5) != null) {
            var12 = DataResult.success((Supplier)() -> var2.get(var5), Lifecycle.stable());
         } else {
            var12 = var9.map((var2x) -> () -> var2.get(var5));
         }
   ...
   private <E> RegistryReadOps.ReadCache<E> readCache(ResourceKey<? extends Registry<E>> var1) {
      return (RegistryReadOps.ReadCache<E>)this.readCache.computeIfAbsent(var1, var0 -> new RegistryReadOps.ReadCache());
   }

   protected <E> DataResult<Registry<E>> registry(ResourceKey<? extends Registry<E>> var1) {
      return (DataResult)this.registryHolder
         .registry(var1)
   ...
   public interface ResourceAccess {
      ...
      static RegistryReadOps.ResourceAccess forResourceManager(final ResourceManager var0) {
         return new RegistryReadOps.ResourceAccess() {
            ...
            public Collection<ResourceLocation> listResources(ResourceKey<? extends Registry<?>> var1) {
               return var0.listResources(var1.location().getPath(), var0xx -> var0xx.endsWith(".json"));
            }
            ...
            public <E> DataResult<Pair<E, OptionalInt>> parseElement(
               ...
               try (Resource var7 = var0.getResource(var6)) {
                  Reader var9 = new InputStreamReader(var7.getInputStream(), StandardCharsets.UTF_8);
                  ...
                  try {
                     ...
                     var13 = var4.parse(var1, var12).map((var0x) -> Pair.of(var0x, OptionalInt.empty()));
                  ...
                  return var13;
               } catch (JsonIOException | JsonSyntaxException | IOException var7_1) {
                  return DataResult.error("Failed to parse " + var6 + " file: " + ((Exception)var7_1).getMessage());
               }
            }
      ...
      public static final class MemoryMap implements RegistryReadOps.ResourceAccess {
         ...
         public <E> void add(RegistryAccess.RegistryHolder var1, ResourceKey<E> var2, Encoder<E> var3, int var4, E var5, Lifecycle var6) {
            DataResult<JsonElement> var7 = var3.encodeStart(RegistryWriteOps.create(JsonOps.INSTANCE, var1), var5);
            Optional<DataResult.PartialResult<JsonElement>> var8 = var7.error();
            if (var8.isPresent()) {
               RegistryReadOps.LOGGER.error("Error adding element: {}", (var8.get()).message());
            } else {
         ...
         public Collection<ResourceLocation> listResources(ResourceKey<? extends Registry<?>> var1) {
            return this.data
               .keySet()
               .stream()
               .filter(var1x -> var1x.isFor(var1))
               .map(var1x -> new ResourceLocation(var1x.location().getNamespace(), var1.location().getPath() + "/" + var1x.location().getPath() + ".json"))
         ...
         public <E> DataResult<Pair<E, OptionalInt>> parseElement(
            ...
            return var5 == null
               ...
               : var4.parse(var1, var5).setLifecycle((Lifecycle)this.lifecycles.get(var3)).map(var2x -> Pair.of(var2x, OptionalInt.of(this.ids.getInt(var3))));
         }
      }
   }
}
