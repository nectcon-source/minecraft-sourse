...
public class UniformInt {
   public static final Codec<UniformInt> CODEC = Codec.either(
         ...
         RecordCodecBuilder.<UniformInt>create(
               var0 -> var0.group(Codec.INT.fieldOf("base").forGetter(var0x -> var0x.baseValue), Codec.INT.fieldOf("spread").forGetter(var0x -> var0x.spread))
                     .apply(var0, UniformInt::new)
            )
            .comapFlatMap(
               var0 -> var0.spread < 0 ? DataResult.error("Spread must be non-negative, got: " + var0.spread) : DataResult.success(var0), Function.identity()
            )
      )
      .xmap(var0 -> (UniformInt)var0.map(UniformInt::fixed, var0x -> var0x), var0 -> var0.spread == 0 ? Either.left(var0.baseValue) : Either.right(var0));
   private final int baseValue;
   private final int spread;

   public static Codec<UniformInt> codec(int var0, int var1, int var2) {
      Function<UniformInt, DataResult<UniformInt>> var3 = var3x -> {
