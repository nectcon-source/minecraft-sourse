...
public class EntityBlockStateFix extends DataFix {
   private static final Map<String, Integer> MAP = DataFixUtils.make(Maps.newHashMap(), var0 -> {
   ...
   public TypeRewriteRule makeRule() {
      ...
      Function<Typed<?>, Typed<?>> var3 = (var1x) -> this.updateBlockToBlockState(var1x, "DisplayTile", "DisplayData", "DisplayState");
      Function<Typed<?>, Typed<?>> var4 = (var1x) -> this.updateBlockToBlockState(var1x, "inTile", "inData", "inBlockState");
      Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>> var5 = DSL.and(DSL.optional(DSL.field("inTile", DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())))), DSL.remainderType());
      Function<Typed<?>, Typed<?>> var6 = (var1x) -> var1x.update(var5.finder(), DSL.remainderType(), Pair::getSecond);
      return this.fixTypeEverywhereTyped("EntityBlockStateFix", var1.getType(References.ENTITY), var2.getType(References.ENTITY), (var4x) -> {
         ...
         var4x = this.updateEntity(var4x, "minecraft:enderman", (var1x) -> this.updateBlockToBlockState(var1x, "carried", "carriedData", "carriedBlockState"));
   ...
   private Typed<?> updateFallingBlock(Typed<?> var1) {
      Type<Either<Pair<String, Either<Integer, String>>, Unit>> var2 = DSL.optional(
         DSL.field("Block", DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())))
      );
      Type<Either<Pair<String, Dynamic<?>>, Unit>> var3x = DSL.optional(DSL.field("BlockState", DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType())));
      Dynamic<?> var4xx = var1.get(DSL.remainderFinder());
      return var1.update(var2.finder(), var3x, var1x -> {
         int var2x = var1x.map(var0x -> (Integer)(var0x.getSecond()).map(var0xx -> var0xx, EntityBlockStateFix::getBlockId), var1xx -> {
            Optional<Number> var2xx = var4xx.get("TileID").asNumber().result();
            return var2xx.map(Number::intValue).orElseGet(() -> var4xx.get("Tile").asByte((byte)0) & 0xFF);
         });
         int var3xx = var4xx.get("Data").asInt(0) & 15;
         return Either.left(Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(var2x << 4 | var3xx)));
      }).set(DSL.remainderFinder(), var4xx.remove("Data").remove("TileID").remove("Tile"));
   }

   private Typed<?> updateBlockToBlockState(Typed<?> var1, String var2, String var3, String var4) {
      Type<Pair<String, Either<Integer, String>>> var5 = DSL.field(
              var2, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString()))
      );
      Type<Pair<String, Dynamic<?>>> var6x = DSL.field(var4, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
      Dynamic<?> var7xx = var1.getOrCreate(DSL.remainderFinder());
      return var1.update(var5.finder(), var6x, var2x -> {
         int var3x = (var2x.getSecond()).map(var0x -> var0x, EntityBlockStateFix::getBlockId);
         int var4x = var7xx.get(var3).asInt(0) & 15;
         return Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(var3x << 4 | var4x));
      }).set(DSL.remainderFinder(), var7xx.remove(var3));
   }
