...
public class IglooMetadataRemovalFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      Type<?> var2x = this.getOutputSchema().getType(References.STRUCTURE_FEATURE);
      return this.writeFixAndRead("IglooMetadataRemovalFix", var1, var2x, IglooMetadataRemovalFix::fixTag);
   }
   ...
   private static <T> Dynamic<T> removeIglooPieces(Dynamic<T> var0) {
      return (Dynamic<T>)var0.asStreamOpt().map(var0x -> var0x.filter(var0xx -> !isIglooPiece(var0xx))).map(var0::createList).result().orElse(var0);
   }
