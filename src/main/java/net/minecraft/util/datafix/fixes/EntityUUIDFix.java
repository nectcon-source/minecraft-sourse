...
public class EntityUUIDFix extends AbstractUUIDFix {
   ...
   private static Dynamic<?> updateFox(Dynamic<?> var0) {
      Optional<Dynamic<?>> var1 = var0.get("TrustedUUIDs")
         .result()
         .map(var1x -> var0.createList(var1x.asStream().map(var0xx -> createUUIDFromML(var0xx).orElseGet(() -> {
                  ...
                  .map(
                     var0xx -> var0xx.update(
                           ...
                           var1x -> var0xx.createList(
                                 var1x.asStream().map(var0xxxx -> replaceUUIDLeastMost(var0xxxx, "UUID", "UUID").orElse(var0xxxx))
                              )
                        )
                  )
            )
