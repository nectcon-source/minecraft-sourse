...
public class VillagerRebuildLevelAndXpFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      OpticFinder<?> var2x = DSL.namedChoice("minecraft:villager", var1);
      OpticFinder<?> var3xx_ = var1.findField("Offers");
      Type<?> var4xxx = var3xx_.type();
      OpticFinder<?> var5xxxx = var4xxx.findField("Recipes");
      ListType<?> var6xxxxx = (ListType)var5xxxx.type();
      OpticFinder<?> var7xxxxxx = var6xxxxx.getElement().finder();
      return this.fixTypeEverywhereTyped(
         "Villager level and xp rebuild", this.getInputSchema().getType(References.ENTITY), var5x -> var5x.updateTyped(var2x, var1, var3xx -> {
               ...
               if (var5xx == 0 || var5xx == 1) {
                  int var7xxxx = var3xx.getOptionalTyped(var3xx_).flatMap(var1xxx -> var1xxx.getOptionalTyped(var5xxxx)).map(var1xxx -> var1xxx.getAllTyped(var7xxxxxx).size()).orElse(0);
                      var5xx = Mth.clamp(var7xxxx / 2, 1, 5);
                  if (var5xx > 1) {
                      var6xx = addLevel(var3xx, var5xx);
                  }
               }
   
               Optional<Number> var8 = var4xx.get("Xp").asNumber().result();
               if (!var8.isPresent()) {
                   var6xx = addXpFromLevel(var6xx, var5xx);
               }
