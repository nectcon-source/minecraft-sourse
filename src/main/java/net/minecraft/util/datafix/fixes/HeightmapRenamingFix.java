...
public class HeightmapRenamingFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("HeightmapRenamingFix", var1, var2x_ -> var2x_.updateTyped(var2x, var1xx -> var1xx.update(DSL.remainderFinder(), this::fix)));
   }

   private Dynamic<?> fix(Dynamic<?> var1) {
      Optional<? extends Dynamic<?>> var2 = var1.get("Heightmaps").result();
      if (!var2.isPresent()) {
         return var1;
      } else {
         Dynamic<?> var3 = (Dynamic)var2.get();
         Optional<? extends Dynamic<?>> var4x = var3.get("LIQUID").result();
         if (var4x.isPresent()) {
            var3 = var3.remove("LIQUID");
            var3 = var3.set("WORLD_SURFACE_WG", (Dynamic)var4x.get());
         }

         Optional<? extends Dynamic<?>> var5 = var3.get("SOLID").result();
         if (var5.isPresent()) {
         ...
         Optional<? extends Dynamic<?>> var6 = var3.get("LIGHT").result();
         if (var6.isPresent()) {
            var3 = var3.remove("LIGHT");
            var3 = var3.set("LIGHT_BLOCKING", (Dynamic)var6.get());
         }

         Optional<? extends Dynamic<?>> var7 = var3.get("RAIN").result();
         if (var7.isPresent()) {
