...
public class MobSpawnerEntityIdentifiersFix extends DataFix {
   ...
   private Dynamic<?> fix(Dynamic<?> var1) {
      if (!"MobSpawner".equals(var1.get("id").asString(""))) {
         ...
         Optional<String> var2 = var1.get("EntityId").asString().result();
         if (var2.isPresent()) {
            Dynamic<?> var3x = (Dynamic)DataFixUtils.orElse(var1.get("SpawnData").result(), var1.emptyMap());
            var3x = var3x.set("id", var3x.createString(var2.get().isEmpty() ? "Pig" : var2.get()));
            var1 = var1.set("SpawnData", var3x);
            var1 = var1.remove("EntityId");
         }

         Optional<? extends Stream<? extends Dynamic<?>>> var6 = var1.get("SpawnPotentials").asStreamOpt().result();
         if (var6.isPresent()) {
            var1 = var1.set( "SpawnPotentials",
                    var1.createList(var6.get()
                     .map(
                        var0 -> {
                           Optional<String> var1x = var0.get("Type").asString().result();
                           if (var1x.isPresent()) {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("MobSpawnerEntityIdentifiersFix", this.getInputSchema().getType(References.UNTAGGED_SPAWNER), var1, var2 -> {
         ...
         DataResult<? extends Pair<? extends Typed<?>, ?>> var4x = var1.readTyped(this.fix(var3));
         return !var4x.result().isPresent() ? var2 : (Typed)((Pair)var4x.result().get()).getFirst();
      });
   }
}
