...
public class OptionsKeyLwjgl3Fix extends DataFix {
   private static final Int2ObjectMap<String> MAP = DataFixUtils.make(new Int2ObjectOpenHashMap(), var0 -> {
   ...
   public TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped(
         ...
         var0 -> var0.update(
               DSL.remainderFinder(), var0x -> (Dynamic)var0x.getMapValues().map(var1 -> var0x.createMap((Map)var1.entrySet().stream().map(var0xxx -> {
                        if (((Dynamic)var0xxx.getKey()).asString("").startsWith("key_")) {
                           int var1x = Integer.parseInt(((Dynamic)var0xxx.getValue()).asString(""));
                           if (var1x < 0) {
                              int var4xx = var1x + 100;
                              String var3x;
                              if (var4xx == 0) {
                                 var3x = "key.mouse.left";
                              } else if (var4xx == 1) {
                                 var3x = "key.mouse.right";
                              } else if (var4xx == 2) {
                                 var3x = "key.mouse.middle";
                              } else {
                                 var3x = "key.mouse." + (var4xx + 1);
                              }
      
                              return Pair.of(var0xxx.getKey(), ((Dynamic)var0xxx.getValue()).createString(var3x));
                           } else {
                              String var2 = MAP.getOrDefault(var1x, "key.unknown");
                              return Pair.of(var0xxx.getKey(), ((Dynamic)var0xxx.getValue()).createString(var2));
                           }
                        } else {
