...
public class StatsCounterFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped(
         "StatsCounterFix",
         this.getInputSchema().getType(References.STATS), var1,
         var2 -> {
            Dynamic<?> var3 = (Dynamic)var2.get(DSL.remainderFinder());
            Map<Dynamic<?>, Dynamic<?>> var4x = Maps.newHashMap();
            Optional<? extends Map<? extends Dynamic<?>, ? extends Dynamic<?>>> var5xx = var3.getMapValues().result();
            if (var5xx.isPresent()) {
               for(Entry<? extends Dynamic<?>, ? extends Dynamic<?>> var7 : var5xx.get().entrySet()) {
                  if (((Dynamic)var7.getValue()).asNumber().result().isPresent()) {
                     String var8xxx = ((Dynamic)var7.getKey()).asString("");
                     if (!SKIP.contains(var8xxx)) {
                        String var9xxxx;
                        String var10xxxxx;
                        if (CUSTOM_MAP.containsKey(var8xxx)) {
                           var9xxxx = "minecraft:custom";
                           var10xxxxx = CUSTOM_MAP.get(var8xxx);
                        } else {
                           int var11xxxx = StringUtils.ordinalIndexOf(var8xxx, ".", 2);
                           if (var11xxxx < 0) {
                              continue;
                           }
   
                           String var12xxxx = var8xxx.substring(0, var11xxxx);
                           if ("stat.mineBlock".equals(var12xxxx)) {
                              var9xxxx = "minecraft:mined";
                              var10xxxxx = this.upgradeBlock(var8xxx.substring(var11xxxx + 1).replace('.', ':'));
                           } else if (ITEM_KEYS.containsKey(var12xxxx)) {
                              var9xxxx = ITEM_KEYS.get(var12xxxx);
                              String var13xxxx = var8xxx.substring(var11xxxx + 1).replace('.', ':');
                              String var14xxxxx = this.upgradeItem(var13xxxx);
                              var10xxxxx = var14xxxxx == null ? var13xxxx : var14xxxxx;
                           } else {
                              if (!ENTITY_KEYS.containsKey(var12xxxx)) {
                                 continue;
                              }

                              var9xxxx = ENTITY_KEYS.get(var12xxxx);
                              String var17xxxx = var8xxx.substring(var11xxxx + 1).replace('.', ':');
                              var10xxxxx = ENTITIES.getOrDefault(var17xxxx, var17xxxx);
                           }
                        }
   
                        Dynamic<?> var15xxxx = var3.createString(var9xxxx);
                        Dynamic<?> var16xxxxx = (Dynamic)var4x.computeIfAbsent(var15xxxx, var1xx -> var3.emptyMap());
                        var4x.put(var15xxxx, var16xxxxx.set(var10xxxxx, (Dynamic)var7.getValue()));
                     }
                  }
               }
            }
   
            return (Typed)((Pair)var1.readTyped(var3.emptyMap().set("stats", var3.createMap(var4x)))
                  .result()
